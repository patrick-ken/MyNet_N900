...
static int listfile(const PureFileInfo * const fi,  const char *name)
{
    ...
    if (fi == NULL) {
        ...
        if ( ( !strcmp(path ,"/")) || 
		 ( !strcmp(path ,"/mnt")) )
		{
			if ( ( !strcmp(name ,"HD_a4")) || ( !strcmp(name ,"HD_b4")) )
				return 0;
		}
        memset( ShareName , 0 ,1024);
//jack20071203+
        nwd = charset_fs2client(name);
        //GetShareName((char *)nwd , ShareName);
        strcpy(ShareName,nwd);
        free(nwd);
        //GetShareName((char *)name , ShareName);
        //printf("ShareName =%s\n",name);
    }
...
//                        sizeof_nameline))
        ...
        if (S_ISREG(st.st_mode) ||
            ...
            //addfile(name, suffix);  //jack20070904
            addfile(ShareName, suffix);
        }
    }
    return rval;
}
