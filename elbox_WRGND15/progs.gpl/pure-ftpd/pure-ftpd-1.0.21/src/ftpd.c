...
#include "pure_ftpd_mediaserver.h" 	//+Space10232007
#include <sys/ipc.h>		//+Space01222008
#include <sys/types.h>		//+Space01222008
#include <sys/shm.h>		//+Space01222008

void GetDeviceInfo(void);
void ChangeToRealPath(char *path );
void ChangeToVirtualPath(char *path);
void CleanString(char *string);
void GetShareName(char *DeviceName , char *ShareName );
void GetDeviceName(char *ShareName , char *DeviceName );

#define ANON_CAN_RESUME	1
#define ANON_CAN_CHANGE_PERMS	1
#define ANON_CAN_CHANGE_UTIME	1
#define ANON_CAN_DELETE	1
#define ANON_CAN_RENAME
...
int ClientDisplayNonUTF8 = 0; //jack20070816+

int allowed = 0;
char IE_send_utf8 = 0; //jack20080110+

//----------------------------------------------------------------------------------
//+Space10222007 for Updating UPnP's and Itune's DB dynamically
extern void MediaServer_Add_Del_File(const char *filename, int op_type, ino_t id);
extern void MediaServer_Rename_FIle(const char *from, const char *to, ino_t id);
extern void MediaServer_Rename_Dir(const char *from, const char *to);
extern int InitNASMediaFolder(void);
//char RNfrom_wd[512] = ""; //record the work dir of "renameform" file
int shmid = -1;		//+Space01222008
char *shmptr;		//+Space01222008
char F_iTuneFolder[SHARE_FOLDER_MAX_LEN];		//+Space01222008  fathor process's shared folder infomation
char F_UpnpavFolder[SHARE_FOLDER_MAX_LEN];		//+Space01222008
char usr_path[512] = ""; //+Space01222008
//Space10222007+
//----------------------------------------------------------------------------------
...
//#ifdef WITH_RFC2640 //jack20071203
...
//	}
//    } else 
    if (utf8 <= 0 && strcasecmp(charset_client, charset_fs) != 0) {
...
static int checknamesanity(const char *name, int dot_ok)
{
    ...
    while (*namepnt != 0) {
#if 0 //jack20070413+ for specically char   	
                 ...
                 char *target, size_t target_size)
{
    ...
    struct stat st;
    int ret = 0;
	char ShareName[MAXPATHLEN + 1];
    char path[MAXPATHLEN + 1];  //jack20061206+
    char *nwd= NULL; //jack20071203
	...
	if ( ( !strcmp(path ,"/")) || 
	 ( !strcmp(path ,"/mnt")) )
	{
		if ( ( !strcmp(file ,"HD_a4")) || ( !strcmp(file ,"HD_b4")) )
		{
			return -1;
		}
	}
    memset( ShareName , 0 ,MAXPATHLEN+1);
//jack20071203+
    nwd = charset_fs2client(file);
    //GetShareName((char *)nwd , ShareName);
    strcpy( ShareName , nwd );
    free(nwd);
    //printf("ShareName =%s\n",name);
...
void douser(const char *username)
{
	...
	if (! strcasecmp(username, "anonymous"))
		getUsrPath("ftp", usr_path);
	else
		getUsrPath(username, usr_path);
	//Space01222008+
	...
	if (!strcasecmp(username, "nobody"))
		return;
	if (!strcasecmp(username, "root"))
		return;	
		
	if (strcasecmp(username, "anonymous"))
	{		
		strcpy(ftpuser,username);
		strcat(ftpuser , ":x:");
		...
		{
			//printf("buf = %s\n",buf);
			if (strstr(buf ,ftpuser))
			{
				if (!strstr(buf, "/home/ftp"))
					SearchUser = 1;
				break;
			}
			fgets(buf , 1024 , fp);
		...
		//printf(" SearchUser  = %d\n",SearchUser);
		//use_anonymous = 1; //jack20050407+
	}
	else
	{
		strcpy(ftpuser,"ftp:x:");
	...
	//jack20070816+
	if ( (fp = fopen("/tmp/ClientDisplayNonUTF8","r")) != NULL )
	{
		fclose(fp);
		ClientDisplayNonUTF8 = 1;
	}
	//jack20070816 edn
	
	//jack20050322+
	if ( (fp = fopen("/etc/ftp_tbl","r")) != NULL )
	{
		fgets(buf , 80 , fp);
		do
		{
...
# ifndef NON_ROOT_FTP
#  ifndef HAVE_SYS_FSUID_H
		//+++ michael_lee: (2012.03.06 15:56:50)
		// no change to user's privelege, run as root
		if (no_seteuid != 1)
		{
			if (seteuid(authresult.uid) != 0) {
				_EXIT(EXIT_FAILURE);
...
void docwd(const char *tmp_dir)
{
	...
	char dir[MAXPATHLEN + 256U]; //jack20071203+
//#ifdef WITH_RFC2640
    char *nwd;
//#endif
    ...
    //jack20071203+
    memset( dir , 0 , MAXPATHLEN + 256U );
    nwd = (char *)charset_client2fs(tmp_dir);
    strcpy( dir , nwd);
    free(nwd);
//    ChangeToRealPath((char *)dir);	//jack20060927+
    ...
    if (chdir(where) != 0) {
        ...
        if (SNCHECK(snprintf(buffer, sizeof buffer,
                             ...
                             tmp_dir, strerror(errno)), sizeof buffer)) {
            _EXIT(EXIT_FAILURE);
        }
        
        //jack20070816+ for ?Xù??
		if ( ( strcasecmp(charset_client,"gb2312") == 0 ) || ( strcasecmp(charset_client,"cp1251") == 0 )
			|| ( ( strcasecmp(charset_client,"cp1250") == 0 ) && ( IE_send_utf8 == 1 )) )
		{
			addreply(250, MSG_CURRENT_DIR_IS, tmp_dir);
		}
		else
		{
...
#if	1 //jack20071203
    nwd = charset_fs2client(wd);
//    ChangeToVirtualPath(nwd);  //jack20070904+
    addreply(250, MSG_CURRENT_DIR_IS, nwd);
    free(nwd);
#else
...
void dochmod(char *name, mode_t mode)
{
    ...
    if ( allowed == 0 )
	{
		addreply_noformat(550, "Users can not change perms");
        return;
	}
	
	memset( tmp_name , 0 , MAXPATHLEN);
	//jack20071203+
	nwd = (char *)charset_client2fs(name);
	strcpy(name,nwd);
	free(nwd);
...
void dodele(char *tmp_name)
{
	char *nwd = NULL; //jack20071203+
	char name[MAXPATHLEN];
   //--------------------------------------------------------------------------------
    //+Space11022007 for Updating UPnP's and Itune's DB dynamically
    char filename[2048] = "";
    ino_t id;
    int retval = 0;

    if (wd[1] == '\0')
    	sprintf(filename, "%s%s", wd, tmp_name);
    else
    	sprintf(filename, "%s/%s", wd, tmp_name);
    
    retval = getID(filename, &id);
    if (retval == -1)
    {
	MY_PRINT(printf("SPACE------GET Ino ERR!\n");)
	id = (ino_t)(-1);
    }
    //Space11022007+
    //---------------------------------------------------------------------------------
...
//#endif
	...
	//jack20071203+
	nwd = (char *)charset_client2fs(tmp_name);
	strcpy(name,nwd);
	free(nwd);
	
    if (tmp_name == NULL || *tmp_name == 0) {
        addreply_noformat(501, MSG_NO_FILE_NAME);
        return;
    }
    if (checknamesanity(name, dot_write_ok) != 0) {
        addreply(550, MSG_SANITY_FILE_FAILURE, tmp_name);
        return;
    }
    if (keepallfiles != 0) {
#ifdef EPERM
...
# endif            
...
#endif
    addreply(250, MSG_DELE_SUCCESS, tmp_name);
    logfile(LOG_NOTICE, MSG_DELE_SUCCESS, tmp_name);    

    //--------------------------------------------------------------------------------
    //+Space10222007 for Updating UPnP's and Itune's DB dynamically
    char filepath[2048] = "";

    //sprintf(filepath, "/mnt%s/%s", wd, tmp_name);
    if (wd[1] != '\0')
    	sprintf(filepath, "%s%s/%s", usr_path, wd, tmp_name, 2048);
    else
	sprintf(filepath, "%s%s%s", usr_path, wd, tmp_name, 2048);
    MY_PRINT(printf("SPACE------DELE<%s>\n", filepath);)
    MediaServer_Add_Del_File(filepath, DEL, id);
    //Space10222007+
    //---------------------------------------------------------------------------------
    ...
    denied:
    addreply(550, MSG_DELE_FAILED ": %s", tmp_name, strerror(errno));
}
...
# endif
    ...
    if (checknamesanity(tmp_name, dot_read_ok) != 0) {
        addreply(550, MSG_SANITY_FILE_FAILURE, name);
        goto end;
    }
    if ((f = open(tmp_name, O_RDONLY)) == -1) {
...
# ifdef SENDFILE_LINUX
                ...
                do {
			//limit send length to INT_MAX because rw_verify_area doesn't like huge counts (in kernel) (tom, 20100824)
		    //w = sendfile(xferfd, f, &o, (size_t) left);
		    w = sendfile64(xferfd, f, &o, (left > INT_MAX) ? INT_MAX : left);
...
//    }
	...
	if ( allowed == 0 )
	{
		addreply_noformat(550, "Sorry, users are not allowed to create directories");
        return;
    }
...
//	ChangeToRealPath((char *)tmp_name);	//jack20080109+
	
    if (checknamesanity(tmp_name, dot_write_ok) != 0) {
...
#endif
    if ((mkdir(tmp_name, (mode_t) (0777 & ~u_mask_d))) < 0) {
#ifdef QUOTAS
        (void) quota_update(&quota, -1LL, 0LL, &overflow);
#endif
    ...
    sprintf(filepath, "/mnt%s/%s", wd, name, 2048);
printf("SPACE------MKD<%s>\n", filepath);
    //Space10222007+
    //---------------------------------------------------------------------------------
#endif
...
void dormd(char *name)
{
	...
	if ( allowed == 0 )
	{
        addreply_noformat(550, "Sorry, users are not allowed to remove directories");
        return;
    }
...
void dostor(char *tmp_name, const int append, const int autorename)
{
    int f;
    char *p;
	...
	if ( allowed == 0 )
	{
		addreply_noformat(553, "Users may not upload files");
        goto end;
	}
	//jack20070830 end
...
//jack20071203+
    ...
    if (checknamesanity(name, dot_write_ok) != 0 ||
        ...
        addreply(553, MSG_SANITY_FILE_FAILURE, tmp_name);
    ...
    MY_PRINT(printf("tmp_name<%s>\n", tmp_name);)
    MY_PRINT(printf("wd<%s>\n", wd);)
    
    if (atomic_file)
    {
    	if (wd[1] != '\0')
    		sprintf(filepath, "%s%s/%s", usr_path, wd, atomic_file, 2048);
	else
		sprintf(filepath, "%s%s%s", usr_path, wd, atomic_file, 2048);
    }
    else
    ...
    MY_PRINT(printf("SPACE------STOR<%s>\n", filepath);)
    MediaServer_Add_Del_File(filepath, ADD, 0);
    //Space10222007+
    //---------------------------------------------------------------------------------
}
...
void domdtm(const char *name)
{
    ...
    strcpy( tmp_name , nwd);
    free(nwd);
//    ChangeToRealPath((char *)tmp_name);	//jack20060927+

    if (!name || !*name) {
        addreply_noformat(500, MSG_MISSING_ARG);
    } else if (stat(tmp_name, &st)) {
#ifdef DEBUG
        if (debug != 0) {
...
void dornfr(char *name)
{
    ...
    if ( allowed == 0 )
    {
        addreply_noformat(550, "Users are not allowed to move/rename files");
        return;
    }
    ...
    if ((lstat(tmp_name, &st)) == 0) {
        if (renamefrom != NULL) {
            addreply_noformat(0, MSG_RENAME_ABORT);
            (void) free(renamefrom);
        }
        if ((renamefrom = strdup(tmp_name)) == NULL) {
            die_mem();
        }
/* //mark by Space01222008
//+Space11132007
if (getcwd(RNfrom_wd, sizeof(RNfrom_wd) - (size_t) 1U) == NULL)
{
        RNfrom_wd[0] = '/';
        RNfrom_wd[1] = 0;
        MY_PRINT(printf("SPACE------FAIL!\n");)
}
MY_PRINT(printf("SPACE------rnfrom_wd<%s>\n", RNfrom_wd);)
//Space11132007+
*/ //mark by Space01222008
...
void dornto(char *name)
{
     ...
     */
    if ((rename(renamefrom, tmp_name)) < 0) {
    ...
    sprintf(renameFR, "%s%s", usr_path, renamefrom);
    sprintf(renameTO, "%s%s", usr_path, tmp_name);

    MY_PRINT(printf("SPACE------RNFR<%s> RNTO<%s>\n", renameFR, renameTO);)

    if (stat(name, &buf) == -1)
    {
    	MY_PRINT(printf("SPACE------STAT ERR!\n");)
	return;
    }

    if (S_ISDIR(buf.st_mode))
    	MediaServer_Rename_Dir(renameFR, renameTO);
    else
    	MediaServer_Rename_FIle(renameFR, renameTO, buf.st_ino);
    //Space10222007+
    //---------------------------------------------------------------------------------
...
void doopts(char *args)
{
	...
	return;
     ...
     //jack20080110+ for IE send UTF8 command
    if (strncasecmp("utf8 ", args, 5) == 0) 
    {
    	IE_send_utf8 = 1;
	}	
...
# endif
    ...
    while ((fodder =
        ...
        //+++ michael_lee: (2012.03.07 09:45:19)
        case '7': {
            no_seteuid = 1;
            break;
        }
...
# endif
    ...
    memset(device_info , 0 , sizeof(DEVICE_INFO)*10);
	GetDeviceInfo();
...
void GetDeviceInfo(void)
{
	...
	int check_flag=0;
	memset(buf , 0 , 80 );
	if ( ( fp = fopen("/etc/shared_name","r")) != NULL )
	{
		fgets(buf,80,fp);
		do
		{
			...
			{
				*p1 = 0x00;
				strcpy(device_info[device_info_num].ShareName,buf);
				*p1 = ' ';
				check_flag++;
			}
			...
			{
				p1++;
				strcpy(device_info[device_info_num].DeviceName,p1);
				CleanString(device_info[device_info_num].DeviceName);
				check_flag++;
			}
			//printf("SahreName = %s\n",device_info[device_info_num].ShareName);
			//printf("DeviceName = %s\n",device_info[device_info_num].DeviceName);
			if ( check_flag >= 2)
			{
				check_flag =0;
				device_info_num++;
	...
	//printf("device_info_num = %d\n",device_info_num);
}

void ChangeToRealPath(char *path )
{
	char *p1,*p2;
	char ShareName[MAXPATHLEN + 4];
	char DeviceName[MAXPATHLEN + 4];
	char tmpbuf[MAXPATHLEN + 4];
	
	//printf("path = %s\n",path);
	memset(DeviceName , 0 , MAXPATHLEN + 4);
	memset(tmpbuf , 0 , MAXPATHLEN+4);
	...
	if ( ( path[0] == '/') && ( path[1] == 'm') && ( path[2] == 'n') && ( path[3] == 't') )
	{
		...
		{
			*p2 = 0x00;
			memset( ShareName , 0 ,MAXPATHLEN + 4);
			strcpy( ShareName , p1 );
			//printf("ShareName = %s\n",ShareName);
			*p2 = '/';
			p2++;
			GetDeviceName(ShareName , DeviceName);
			if ( DeviceName[0] != 0x00 )
			{
				sprintf(tmpbuf,"/mnt/%s/%s",DeviceName,p2);
				strcpy(path,tmpbuf);
			}
		}
		else
		{
			...
			if ( DeviceName[0] != 0x00 )
			{
				sprintf(tmpbuf,"/mnt/%s",DeviceName);
				strcpy(path,tmpbuf);
			}
		}
	}
	else if ( ( path[0] == '/') && ( path[1] == 'V') && ( path[2] == 'o') && ( path[3] == 'l') &&
			  ( path[4] == 'u') && ( path[5] == 'm') && ( path[6] == 'e') && ( path[7] == '_') )
	{
		p1 = path+1;
		if ( ( p2 = strchr(p1 ,'/')) != NULL )
		{
			...
			*p2 = '/';
			...
			if ( DeviceName[0] != 0x00 )
			{
				sprintf(tmpbuf,"/%s/%s",DeviceName,p2);
				strcpy(path,tmpbuf);
			}
		}
		else
...
void ChangeToVirtualPath(char *path)
{
	...
	if ( strcmp( path , "/" ) )
	{
		...
		if ( ( p2 = strchr(p1 ,'/')) != NULL )
		{
			*p2 = 0x00;
			memset( DeviceName , 0 ,MAXPATHLEN + 4);
			strcpy( DeviceName , p1 );
			//printf("DeviceName = %s\n",DeviceName);
			*p2 = '/';
			p2++;
			GetShareName(DeviceName , ShareName );
			if ( ShareName[0] != 0x00 )
			{
				sprintf(tmpbuf,"/%s/%s",ShareName,p2);
				strcpy(path,tmpbuf);
			}
		}
		else
...
void CleanString(char *string)
{
	...
	p = strchr(string , 0x0a );
	if ( p != NULL )
		*p = 0x00;
	
	p = strchr(string , 0x0d );
	if ( p != NULL )
		*p = 0x00;
}
...
void GetShareName(char *DeviceName , char *ShareName )
{
	int i;
	//printf("DeviceName = %s\n",DeviceName);
	strcpy( ShareName , DeviceName);
	for ( i=0;i<device_info_num;i++)	
	{
		if ( !strcmp( device_info[i].DeviceName , DeviceName ) )
		{
			strcpy(ShareName , device_info[i].ShareName );
			break;
		}
	}
	//printf("ShareName = %s\n",ShareName);
...
//	printf("ShareName = %s\n",ShareName);
	...
	for ( i=0;i<device_info_num;i++)	
	{
		//printf("device_info[i].ShareName = %s\n",device_info[i].ShareName);
		if ( strstr( ShareName , device_info[i].ShareName ) )
		{
			strcpy(DeviceName , device_info[i].DeviceName );
			break;
		}
	}
...
#if 0
//+Space10222007 for Updating UPnP's and Itune's DB dynamically
void MediaServer_Add_Del_File(const char *filename, int op_type)
{
	char path[2048] = "";
	char *ext = NULL;
	char *cmd_buf = NULL;
	...
	if (path_type == AUDIOTYPE)
		snprintf(cmd_buf,cmd_buf_len,"MediaServer %s \"%s\"\n", op, path);
	else if (path_type == VIDEOTYPE)
		snprintf(cmd_buf,cmd_buf_len,"MediaServer %s -M \"%s\"\n", op, path);
	else if (path_type == IMAGETYPE)
		snprintf(cmd_buf,cmd_buf_len,"MediaServer %s -M \"%s\"\n", op, path);
	else
		return;
//printf("SPACE------CMD_BUF<%s>\n", cmd_buf);
...
void UpdateScheduler(char *cmd_buf)
{
	...
	fdsch = open( SCHEDULER_BK_FILE, O_APPEND|O_WRONLY|O_CREAT, (mode_t) 0700  );
//printf("SPACE------OPEN FILE<%s> FD<%d>\n", SCHEDULER_BK_FILE, fdsch);
	if (fdsch == -1)
		return;
//printf("SPACE------OPEN FILE<%s> DONE!\n", SCHEDULER_BK_FILE);	
	write_wlock(fdsch);
	write(fdsch, cmd_buf, strlen(cmd_buf) );
	clear_lock(fdsch);
	close(fdsch);

	fdsch = open( SCHEDULER_FILE, O_APPEND|O_WRONLY|O_CREAT, (mode_t) 0700  );
	if (fdsch == -1)
		return;
