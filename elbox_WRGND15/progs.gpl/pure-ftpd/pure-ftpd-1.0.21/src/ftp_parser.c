...
/*
 * if corrent directory is '/' return 1 others return 0
 */
static int isrootwd()
{
	int ret = 0;
...
//	}
//    } else 
    if (utf8 <= 0 && strcasecmp(charset_fs, charset_client) != 0) {
...
#endif
		//+++ michael_lee: (2012.03.06 11:19:21)
		if (root_dir_read_only == 1)
		{
			if ( !strcmp(cmd, "dele") ||
					!strcmp(cmd, "mkd") || !strcmp(cmd, "xmkd") ||
					!strcmp(cmd, "rmd") || !strcmp(cmd, "xrmd") ||
					!strcmp(cmd, "stor") ||
					!strcmp(cmd, "appe") ||
					!strcmp(cmd, "rnfr") ||
					!strcmp(cmd, "rnto") )
			{
				if (isrootwd() == 1)
				{
					addreply_noformat(553, MSG_PROBABLY_DENIED);
					goto wayout;
				}
			}
		}
...
//		ChangeToVirtualPath(nwd);	//jack20070423+	
		addreply(257, "\"%s\" " MSG_IS_YOUR_CURRENT_LOCATION, nwd);
		free(nwd);
#else
//                ChangeToVirtualPath(wd);	//jack20070423+	
