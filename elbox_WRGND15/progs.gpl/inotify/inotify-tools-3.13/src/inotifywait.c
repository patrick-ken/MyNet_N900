...
char * MediaServerComm = "M"; //default M: uPNP, I: iTune
...
//ino_t getID(char *path);
bool UpdateDB(int updateFlag, char *FilePath1, char *FilePath2,char *cb_script);
...
int Transfer_Path(char *read_path, char *full_path, int rootflag)
{
	...
	if ((p1 = strchr(read_path, '/')) != NULL)		
	{
		*p1 = 0x00;
		...
		{
			if (rootflag == 1)
				sprintf(full_path, "/mnt/vg%s/lvnas_default/", read_path);
			else
				sprintf(full_path, "/mnt/vg%s/lvnas_default/%s", read_path, p1);
		}
		else
		{
			if (rootflag == 1)
				sprintf(full_path, "/mnt/md%s/", read_path);
			else
				sprintf(full_path, "/mnt/md%s/%s", read_path, p1);
		}
		p1--;
		*p1 = '/';
	}
...
int main(int argc, char ** argv)
{
	...
	if ( !parse_opts(&argc, &argv, &events, &monitor, &quiet, &timeout,
	                 ...
	                 &regex, &iregex,&cb_script) ) {
		return EXIT_ERROR;
	}
	
#if 1 //Added by Ben 2008.04.03
	char* p =	strtok(myname, "/");
	do
	{
		if (strcmp(p, "inotify_uPNP") == 0 )
		{
			if (quiet == 0) printf("Server Type = uPNP\n");
			MediaServerComm = "M"; //UPNP Server
			break;
		}
		else if (strcmp(p, "inotify_iTune") == 0 )
		{
			if (quiet == 0) printf("Server Type = iTune\n");
			MediaServerComm = "I"; //iTune Server
			break;
		}
	}	while(p = strtok(NULL, "/"));
#endif
	...
	if ((regex && !inotifytools_ignore_events_by_regex(regex, REG_EXTENDED) ) ||
		  (iregex && !inotifytools_ignore_events_by_regex(iregex, REG_EXTENDED|REG_ICASE)))
	{
...
#if 1 //Added by Ben 2008.06.26
	char full_path[512];
	memset(full_path, 0x00, sizeof(full_path));
	if (strcmp(argv[0], "root") == 0)
		strcpy(full_path, "/mnt/");
	else
		strcpy(full_path,argv[0]);
//#ifndef DIR685NAS	
//		Transfer_Path(argv[0], full_path, 0);
//#endif		
	argv[0] = full_path;
#endif
	...
	if ( !quiet )
	{
		...
		if ( recursive )
			fprintf( stderr, "Beware: since -r was given, this may take a while!" );
	...
	do {
		...
		if ( quiet < 2 )
		{
			if ( csv ) {
				...
				;//inotifytools_printf( event, "Event:%e, filename = %w%f\n" );
			}
		}
		...
		if ( monitor && recursive )
		{
			if ((event->mask & IN_CREATE) || (!moved_from && (event->mask & IN_MOVED_TO)))
			{
				...
				UpdateDB(UPDATE_ADD, new_file, '\0',cb_script);
				free( new_file );
			} // IN_CREATE
			...
			else if (event->mask & IN_MOVED_TO)
			{
				if ( moved_from )
				{
					...
					UpdateDB(UPDATE_RENAME, moved_from, new_name,cb_script);
			...
			else if (event->mask & IN_DELETE)
			{
				...
				if (event->mask & IN_ISDIR )
				{
					...
					UpdateDB(UPDATE_DELETE_DIR, del_name, '\0',cb_script);
				}
				else
				{
					nasprintf( &del_name, "%s%s", inotifytools_filename_from_wd( event->wd ), event->name );
					UpdateDB(UPDATE_DELETE, del_name, '\0',cb_script);
				}
...
bool UpdateDB(int updateFlag, char *FilePath1, char *FilePath2,char *cb_script)
{
	if (!updateFlag || !FilePath1 || !cb_script) return 0;
	...
	else if (pid == 0) 
	{
		...
		{
			case UPDATE_ADD:
				sprintf(File,"%s%s",FilePath1, (isdir(FilePath1) ? "/" : "") );
				execlp(cb_script, cb_script, "ADD",File, NULL);
				exit(1);
				break;
			
			case UPDATE_DELETE:
				execlp(cb_script, cb_script, "DEL",FilePath1, NULL);
				exit(1);
				break;
		
			case UPDATE_DELETE_DIR:
				execlp(cb_script, cb_script, "DEL_DIR",FilePath1, NULL);
				exit(1);
				break;
			
			case UPDATE_RENAME:
				execlp(cb_script, cb_script, "RENAME",FilePath1,FilePath2, NULL);
				exit(1);
				break;
		}			
	}
...
bool parse_opts(
  ...
  char **cb_script
	...
	//marco
	long_opts[13].name = "script";
	long_opts[13].has_arg =1;
	long_opts[13].flag = NULL;
	long_opts[13].val = (int)'s';
