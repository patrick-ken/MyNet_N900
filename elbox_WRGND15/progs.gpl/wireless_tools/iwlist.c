...
#ifdef IWLIST_APSCANN
#define RGDBBUFFER_SIZE     128
#define APSCANN_PATH        "/runtime/wireless/apscanlistinfo/apcscan:"
#define APSCANN_CHAN        "/chan"
#define APSCANN_RSSI        "/rssi"
#define APSCANN_MAC         "/mac"
#define APSCANN_SSID        "/ssid"
#define APSCANN_AUTH        "/auth"
#define APSCANN_TYPE        "/type"
#define APSCANN_BITRATE     "/rate"
#define APSCANN_WPS         "/wps"

#define WPA_PSK_STR         "WPA-PSK"
#define WPA_EAP_STR         "WPA-EAP"
#define WPA2_PSK_STR        "WPA2-PSK"
#define WPA2_EAP_STR        "WPA2-EAP"
#define WPA_AUTO_PSK_STR    "WPA-AUTO-PSK"
#define WPA_AUTO_EAP_STR    "WPA-AUTO-EAP"

#define RSN_ID              0x30
#define WPA_ID              0xdd
#define RSN_EAP             0x000fac01
#define RSN_PSK             0x000fac02
#define WPA                 0x0050f201
#define WPS                 0x0050f204
#define WPA_EAP             0x0050f201
#define WPA_PSK             0x0050f202

typedef unsigned char _u8;
typedef unsigned int  _u32;

_u8 rgdb_val_buf[RGDBBUFFER_SIZE];
_u8 rgdb_path_buf[RGDBBUFFER_SIZE];
#endif /*IWLIST_APSCANN*/
...
#endif /*IWLIST_APSCANN*/
{
  ...
  switch(event->cmd)
    ...
    case SIOCGIWAP:
      ...
      memcpy(rgdb_val_buf, iw_pr_ether(buffer, event->u.ap_addr.sa_data), 17);
      memset(rgdb_path_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, ap_num, APSCANN_MAC);
      xmldbc_set(NULL, 0, rgdb_path_buf, rgdb_val_buf);
      
      /*Initialize wps value*/
      memset(rgdb_path_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, (ap_num), APSCANN_WPS);
      xmldbc_set(NULL, 0, rgdb_path_buf, "0");
#else
      printf("Cell %02d\n   Address: %s\n", ap_num,
	     iw_pr_ether(buffer, event->u.ap_addr.sa_data));
#endif /*IWLIST_APSCANN*/
	...
	iw_print_freq(buffer, freq);
      ...
      if(debug)
        printf("   %s\n", buffer);
      memset(rgdb_val_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_val_buf, "%d", (int)freq);
      memset(rgdb_path_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, (ap_num-1), APSCANN_CHAN);
      xmldbc_set(NULL, 0, rgdb_path_buf, rgdb_val_buf);
#else
	printf("   %s\n", buffer);
#endif /*IWLIST_APSCANN*/
    ...
    case SIOCGIWMODE:
      ...
      {
      	case 1: /*Ad-Hoc*/
      		sprintf(rgdb_val_buf, "%s", "Ad-Hoc");
      		break;
      	case 2: /*Managed*/
      		sprintf(rgdb_val_buf, "%s", "Infrastructure");
      		break;
      	default:
      		sprintf(rgdb_val_buf, "%s", "Unknown");
      		break;
      }
      memset(rgdb_path_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, (ap_num-1), APSCANN_TYPE);
      xmldbc_set(NULL, 0, rgdb_path_buf, rgdb_val_buf);
#else
      printf("   Mode:%s\n",
	     iw_operation_mode[event->u.mode]);
	...
	if(event->u.essid.flags)
      ...
      memset(rgdb_val_buf, 0x0, RGDBBUFFER_SIZE);
      memcpy(rgdb_val_buf, essid, strlen(essid));
      memset(rgdb_path_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, (ap_num-1), APSCANN_SSID);
      xmldbc_set(NULL, 0, rgdb_path_buf, rgdb_val_buf);
#else
	if(event->u.essid.flags)
	  {
	...
	if(event->u.data.flags & IW_ENCODE_DISABLED)
	  ...
	  printf("off\n");
      memset(rgdb_path_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, (ap_num-1), APSCANN_AUTH);
      xmldbc_set(NULL, 0, rgdb_path_buf, "OPEN");
    }else{
	    /* Display the key */
      ...
      memset(rgdb_path_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, (ap_num-1), APSCANN_AUTH);
      xmldbc_set(NULL, 0, rgdb_path_buf, "SHARED");
	  }
#else
	unsigned char	key[IW_ENCODING_TOKEN_MAX];
	if(event->u.data.pointer)
    ...
    case SIOCGIWRATE:
      ...
      if(debug)
        printf("   Bit Rate:%s\n", buffer);
      memset(rgdb_val_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_val_buf, "%d", atol(buffer));
      memset(rgdb_path_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, (ap_num-1), APSCANN_BITRATE);
      xmldbc_set(NULL, 0, rgdb_path_buf, rgdb_val_buf);
#else
      printf("   Bit Rate:%s\n", buffer);
#endif /*IWLIST_APSCANN*/
	...
	iw_print_stats(buffer, &event->u.qual, iwrange, has_range);
      ...
      if(debug)
	    printf("   %s\n", buffer);
      memset(rgdb_val_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_val_buf, "%d", event->u.qual.qual);
      memset(rgdb_path_buf, 0x0, RGDBBUFFER_SIZE);
      sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, (ap_num-1), APSCANN_RSSI);
      xmldbc_set(NULL, 0, rgdb_path_buf, rgdb_val_buf);
#else
	printf("   %s\n", buffer);
#endif /*IWLIST_APSCANN*/
...
#ifdef IWLIST_APSCANN
    {
        ...
        /*Retrieve Infomation Element ID*/
        memset(fieldstr, 0x0, 10);
        memcpy(fieldstr, pie, 2);
        ie_id = strtol(fieldstr, (char **)NULL, 16);
        //printf("ie_id: %x\n", ie_id);

        if(ie_id==RSN_ID){
            //printf("RSN\n");
            /*Retrieve Pairwise cipher suite count*/
            pie += 16;
            memset(fieldstr, 0x0, 10);
            memcpy(fieldstr, (_u8 *)(pie+2), 2);
            memcpy((_u8 *)&fieldstr[2], pie, 2);
            pwcscnt = strtol(fieldstr, (char **)NULL, 16);
            //printf("pwcscnt = %d\n", pwcscnt);

            /*Retrieve Authentication count*/
            pie += 4;
            pie += pwcscnt*8;
            memset(fieldstr, 0x0, 10);
            memcpy(fieldstr, (_u8 *)(pie+2), 2);
            memcpy((_u8 *)&fieldstr[2], pie, 2);
            authcnt = strtol(fieldstr, (char **)NULL, 16);
            //printf("authcnt = %d\n", authcnt);

            /*Retrieve Authentication*/
            pie += 4;
            memset(fieldstr, 0x0, 10);
            memcpy(fieldstr, pie, 8);
            auth = strtol(fieldstr, (char **)NULL, 16);
            //printf("auth = %x\n", auth);

            memset(rgdb_val_buf, 0x0, RGDBBUFFER_SIZE);
            memset(rgdb_path_buf, 0x0, RGDBBUFFER_SIZE);
            sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, (ap_num-1), APSCANN_AUTH);
            xmldbc_get_wb(NULL, 0, rgdb_path_buf, rgdb_val_buf, RGDBBUFFER_SIZE);
            switch(auth){
                case RSN_EAP:
                    if (!memcmp(rgdb_val_buf, WPA_EAP_STR, strlen(WPA_EAP_STR))){
                        memset(rgdb_val_buf, 0x0, RGDBBUFFER_SIZE);
                        //printf("WPA-AUTO-EAP\n");
                        sprintf(rgdb_val_buf, "%s", WPA_AUTO_EAP_STR);
                    }else{
                        //printf("WPA2-EAP\n");
                        sprintf(rgdb_val_buf, "%s", WPA2_EAP_STR);
                    }
                    break;
                case RSN_PSK:
                    if (!memcmp(rgdb_val_buf, WPA_PSK_STR, strlen(WPA_PSK_STR))){
                        memset(rgdb_val_buf, 0x0, RGDBBUFFER_SIZE);
                        //printf("WPA-AUTO-PSK\n");
                        sprintf(rgdb_val_buf, "%s", WPA_AUTO_PSK_STR);
                    }else{
                        //printf("WPA2-PSK\n");
                        sprintf(rgdb_val_buf, "%s", WPA2_PSK_STR);
                    }
                    break;
                default:
                    //printf("unknown\n");
            ...
            sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, (ap_num-1), APSCANN_AUTH);
            xmldbc_set(NULL, 0, rgdb_path_buf, rgdb_val_buf);
        }else if(ie_id==WPA_ID){
            //printf("WPA\n");
            /*Retrieve WPA OUI*/
            pie += 4;
            memset(fieldstr, 0x0, 10);
            memcpy(fieldstr, pie, 8);
            wpa = strtol(fieldstr, (char **)NULL, 16);
            //printf("wpa: %x\n", wpa);
            if (wpa==WPS){
                memset(rgdb_path_buf, 0x0, RGDBBUFFER_SIZE);
                sprintf(rgdb_path_buf, "%s%d%s", APSCANN_PATH, (ap_num-1), APSCANN_WPS);
                xmldbc_set(NULL, 0, rgdb_path_buf, "1");
            }
            if(wpa==WPA){
                /*Retrieve Pairwise cipher suite count*/
                pie += 20;
                ...
                switch(auth){
                    case WPA_EAP:
                        if (!memcmp(rgdb_val_buf, WPA2_EAP_STR, strlen(WPA2_EAP_STR))){
                            memset(rgdb_val_buf, 0x0, RGDBBUFFER_SIZE);
                            //printf("WPA-AUTO-EAP\n");
                            sprintf(rgdb_val_buf, "%s", WPA_AUTO_EAP_STR);
                        }else{
                            //printf("WPA-EAP\n");
                            sprintf(rgdb_val_buf, "%s", WPA_EAP_STR);
                        }
                        break;
                    case WPA_PSK:
                        if (!memcmp(rgdb_val_buf, WPA2_PSK_STR, strlen(WPA2_PSK_STR))){
                            memset(rgdb_val_buf, 0x0, RGDBBUFFER_SIZE);
                            //printf("WPA-AUTO-PSK\n");
                            sprintf(rgdb_val_buf, "%s", WPA_AUTO_PSK_STR);
                        }else{
                            //printf("WPA-PSK\n");
                            sprintf(rgdb_val_buf, "%s", WPA_PSK_STR);
                        }
                        break;
                    default:
                        //printf("unknown\n");
		    ...
		    int		count)		/* Args count */
{
...
#ifdef IWLIST_APSCANN
    xmldbc_del(NULL, 0, "/runtime/wireless/apscanlistinfo");
#endif
  ...
  if(wrq.u.data.length)
      ...
      do
	{
	  ...
	  if(ret > 0)
#ifdef IWLIST_APSCANN
	    ap_num = print_scanning_token(&iwe, ap_num, &range, has_range, debug);
#else
