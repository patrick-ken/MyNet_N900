...
iw_print_freq(char *	buffer,
	      double	freq)
{
	/* Radicalis_choish (04.01.16) */
  if(freq < KILO)
    sprintf(buffer, "Channel:%d", (int)freq);
  else
    {
      if(freq >= GIGA)
	sprintf(buffer, "Frequency:%f GHz", freq / GIGA);
      else
	{
	  if(freq >= MEGA)
	    sprintf(buffer, "Frequency:%f MHz", freq / MEGA);
	  else
	    sprintf(buffer, "Frequency:%f kHz", freq / KILO);
	}
    }
}
...
//  double	rate = bitrate;
  int	rate = bitrate;
  if(rate >= GIGA)
    sprintf(buffer, "%dGb/s", rate / GIGA);
  else
    if(rate >= MEGA)
      sprintf(buffer, "%dMb/s", rate / MEGA);
    else
      sprintf(buffer, "%dkb/s", rate / KILO);
}
