...
#ifdef IWPRIV_APSCAN
#define XMLDB_BF_SIZE       128
#define APSCANN_PATH        "/runtime/phyinf:2/sitesurvey/entry:"
#define APSCANN_CHAN        "/channel"
#define APSCANN_RSSI        "/rssi"
#define APSCANN_MAC         "/macaddr"
#define APSCANN_SSID        "/ssid"
#define APSCANN_ENC         "/encrtype"
#define APSCANN_AUTH        "/authtype"
#define APSCANN_WMODE       "/wlmode"
#define APSCANN_NT          "/bsstype"
#define APSCANN_WPS         "/wps"

#if 0
#define STACONN_STA         "/runtime/phyinf:2/connstatus"
#endif /*0*/

typedef unsigned char _u8;
typedef unsigned int  _u32;

_u8 xmldb_vbf[XMLDB_BF_SIZE];
_u8 xmldb_pbf[XMLDB_BF_SIZE];

struct w_entry{
	unsigned char	Ch[4];
	unsigned char SSID_Len[4];
...
#ifdef IWPRIV_APSCAN
	  /*+++*/
	  if (!strcmp(cmdname, "get_site_survey")){
	  	struct w_entry entry;
	  	int i, j;

	  	for (i=104,j=1; i<wrq.u.data.length-1; i+=103,j++){
	  		memcpy(&entry, &buffer[i], 103);

	  		entry.Ch[3] = 0x0;
	  		entry.SSID_Len[3] = 0x0;
	  		entry.SSID[32] = 0x0;
	  		entry.BSSID[17] = 0x0;
	  		entry.Enc[7] = 0x0;
	  		entry.Auth[9] = 0x0;
	  		...
	  		entry.WPS[3] = 0x0;
				...
				memset(xmldb_pbf, 0x0, XMLDB_BF_SIZE);
				sprintf(xmldb_pbf, "%s%d%s", APSCANN_PATH, j, APSCANN_CHAN);
				memset(xmldb_vbf, 0x0, XMLDB_BF_SIZE);
				sprintf(xmldb_vbf, "%d", atoi(entry.Ch));
				xmldbc_set(NULL, 0, xmldb_pbf, xmldb_vbf);

				memset(xmldb_pbf, 0x0, XMLDB_BF_SIZE);
				sprintf(xmldb_pbf, "%s%d%s", APSCANN_PATH, j, APSCANN_SSID);
				memset(xmldb_vbf, 0x0, XMLDB_BF_SIZE);
				memcpy(xmldb_vbf, entry.SSID, atoi(entry.SSID_Len));
				xmldbc_set(NULL, 0, xmldb_pbf, xmldb_vbf);

				memset(xmldb_pbf, 0x0, XMLDB_BF_SIZE);
				sprintf(xmldb_pbf, "%s%d%s", APSCANN_PATH, j, APSCANN_MAC);
				xmldbc_set(NULL, 0, xmldb_pbf, entry.BSSID);

				memset(xmldb_pbf, 0x0, XMLDB_BF_SIZE);
				sprintf(xmldb_pbf, "%s%d%s", APSCANN_PATH, j, APSCANN_ENC);
				memset(xmldb_vbf, 0x0, XMLDB_BF_SIZE);
				if (!memcmp("NONE", entry.Enc, strlen("NONE"))){
					sprintf(xmldb_vbf, "NONE");
				}else if (!memcmp("WEP", entry.Enc, strlen("WEP"))){
					sprintf(xmldb_vbf, "WEP");
				}else if (!memcmp("TKIP", entry.Enc, strlen("TKIP"))){
					sprintf(xmldb_vbf, "TKIP");
				}else if (!memcmp("AES", entry.Enc, strlen("AES"))){
					sprintf(xmldb_vbf, "AES");
				}else if (!memcmp("TKIPAES", entry.Enc, strlen("TKIPAES"))){
					sprintf(xmldb_vbf, "TKIPAES");
				}else{
					sprintf(xmldb_vbf, "UNKNOW");
				}
				xmldbc_set(NULL, 0, xmldb_pbf, xmldb_vbf);

				memset(xmldb_pbf, 0x0, XMLDB_BF_SIZE);
				sprintf(xmldb_pbf, "%s%d%s", APSCANN_PATH, j, APSCANN_AUTH);
				memset(xmldb_vbf, 0x0, XMLDB_BF_SIZE);
				if (!memcmp("OPEN", entry.Auth, strlen("OPEN"))){
					sprintf(xmldb_vbf, "OPEN");
				}else if (!memcmp("SHARED", entry.Auth, strlen("SHARED"))){
					sprintf(xmldb_vbf, "SHARED");
				}else if (!memcmp("AUTOWEP", entry.Auth, strlen("AUTOWEP"))){
					sprintf(xmldb_vbf, "AUTOWEP");
				}else if (!memcmp("WPA1PSKWPA2PSK", entry.Auth, strlen("WPA1PSKWPA2PSK"))){
					sprintf(xmldb_vbf, "WPA+2PSK");
				}else if (!memcmp("WPA2PSK", entry.Auth, strlen("WPA2PSK"))){
					sprintf(xmldb_vbf, "WPA2PSK");
				}else if (!memcmp("WPAPSK", entry.Auth, strlen("WPAPSK"))){
					sprintf(xmldb_vbf, "WPAPSK");
				}else if (!memcmp("WPANONE", entry.Auth, strlen("WPANONE"))){
					sprintf(xmldb_vbf, "WPANONE");
				}else if (!memcmp("WPA1WPA2", entry.Auth, strlen("WPA1WPA2"))){
					sprintf(xmldb_vbf, "WPA+2");
				}else if (!memcmp("WPA2", entry.Auth, strlen("WPA2"))){
					sprintf(xmldb_vbf, "WPA2");
				}else if (!memcmp("WPA", entry.Auth, strlen("WPA"))){
					sprintf(xmldb_vbf, "WPA");
				}else{
					sprintf(xmldb_vbf, "UNKNOW");
				}
				xmldbc_set(NULL, 0, xmldb_pbf, xmldb_vbf);
				...
				memset(xmldb_pbf, 0x0, XMLDB_BF_SIZE);
				sprintf(xmldb_pbf, "%s%d%s", APSCANN_PATH, j, APSCANN_RSSI);
				memset(xmldb_vbf, 0x0, XMLDB_BF_SIZE);
				sprintf(xmldb_vbf, "%d", atoi(entry.Siganl));
				xmldbc_set(NULL, 0, xmldb_pbf, xmldb_vbf);

				memset(xmldb_pbf, 0x0, XMLDB_BF_SIZE);
				sprintf(xmldb_pbf, "%s%d%s", APSCANN_PATH, j, APSCANN_WMODE);
				memset(xmldb_vbf, 0x0, XMLDB_BF_SIZE);
				if (!memcmp("11b/g/n", entry.W_Mode, strlen("11b/g/n"))){
					sprintf(xmldb_vbf, "11b/g/n");
				}else if (!memcmp("11b/g", entry.W_Mode, strlen("11b/g"))){
					sprintf(xmldb_vbf, "11b/g");
				}else if (!memcmp("11b", entry.W_Mode, strlen("11b"))){
					sprintf(xmldb_vbf, "11b");
				}else if (!memcmp("11a/n", entry.W_Mode, strlen("11a/n"))){
					sprintf(xmldb_vbf, "11a/n");
				}else if (!memcmp("11a", entry.W_Mode, strlen("11a"))){
					sprintf(xmldb_vbf, "11a");
				}else{
					sprintf(xmldb_vbf, "UNKNOW");
				}
				xmldbc_set(NULL, 0, xmldb_pbf, xmldb_vbf);
				...
				memset(xmldb_pbf, 0x0, XMLDB_BF_SIZE);
				sprintf(xmldb_pbf, "%s%d%s", APSCANN_PATH, j, APSCANN_NT);
				memset(xmldb_vbf, 0x0, XMLDB_BF_SIZE);
				if (!memcmp("Ad", entry.NT, strlen("Ad"))){
					sprintf(xmldb_vbf, "AD-HOC");
				}else if (!memcmp("In", entry.NT, strlen("In"))){
					sprintf(xmldb_vbf, "INFRASTRUCTURE");
				}
				xmldbc_set(NULL, 0, xmldb_pbf, xmldb_vbf);

				memset(xmldb_pbf, 0x0, XMLDB_BF_SIZE);
				sprintf(xmldb_pbf, "%s%d%s", APSCANN_PATH, j, APSCANN_WPS);
				memset(xmldb_vbf, 0x0, XMLDB_BF_SIZE);
				if (!memcmp("YES", entry.WPS, strlen("YES"))){
					sprintf(xmldb_vbf, "YES");
				}else if (!memcmp("NO", entry.WPS, strlen("NO"))){
					sprintf(xmldb_vbf, "NO");
				}
				xmldbc_set(NULL, 0, xmldb_pbf, xmldb_vbf);
			}
		}
#if 0
		if (!strcmp(cmdname, "connStatus")){
			memset(xmldb_pbf, 0x0, XMLDB_BF_SIZE);
			sprintf(xmldb_pbf, "%s", STACONN_STA);
			if (!memcmp(buffer, "Disconnected", strlen("Disconnected"))){
				xmldbc_set(NULL, 0, xmldb_pbf, "DISCONNECTED");
			}else{
				xmldbc_set(NULL, 0, xmldb_pbf, "CONNECTED");
			}
		}
#endif /*0*/
	  /*+++*/
 ...
 */
int
//main(int argc, //+++ change to busybox alike way by siyou. 2005.05.04 ---//
