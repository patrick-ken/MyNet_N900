/*
 * $Id: mp3-scanner.c,v 1.2 2006/10/24 10:00:47 aaron Exp $
...
static iconv_result do_convert(const char* to_ces, const char* from_ces, 
  ...
  if (cd == (iconv_t)-1) {
	...
	//debug_Aaron
	DPRINTF(E_WARN,L_SCAN,"iconv_open(to_ces=%s, from_ces=%s) failed!!\n", to_ces, from_ces);
  ...
  if (rc == (size_t)-1) {
	  ...
	  //debug_Aaron
        DPRINTF(E_WARN,L_SCAN,"iconv() failed!!, errno=%d\n", errno);
...
static unsigned char* get_utf8_text(const id3_ucs4_t* native_text) {
  ...
  /* (1) try utf8 -> cp932 */
  rc = do_convert("CP950", "UTF-8", in8, strlen(in8), iconv_buf, MAX_ICONV_BUF);
  if (rc == ICONV_OK) {
    utf8_text = in8;
    free(iconv_buf);
  } else if (rc == ICONV_TRYNEXT) {
    /* (2) try cp932 -> utf8 */
    rc = do_convert("UTF-8", "CP950", in, strlen(in), iconv_buf, MAX_ICONV_BUF);
    if (rc == ICONV_OK) {
      utf8_text = iconv_buf;
    } else if (rc == ICONV_TRYNEXT) {
      /* (3) try euc-jp -> utf8 */
