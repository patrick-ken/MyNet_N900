...
/* add by siyou.  2012/2/24 02:39pm
   Try to assign unique ip address by sta' mac address.
   Check the hashed index address is available or not,
   it check_expired is true, we check all the expired leases as well.
   Maybe this should try expired leases by age... */
u_int32_t find_address_by_chaddr(u_int8_t *chaddr, int check_expired)
{
	u_int32_t addr, ret;
	...
	//by mac address to find ip address.
	hash = ( ( (unsigned int)chaddr[4] << 8) + (unsigned int)chaddr[5]) % (ntohl(server_config.end) - ntohl(server_config.start) +1) ;
