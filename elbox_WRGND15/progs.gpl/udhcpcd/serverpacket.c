...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	/* Bouble - 20120402 - fix adjencent two Discover packets received(such as WDTV) at the same second.
	  * For the secondary packet, It's lease_time_align will be 60 seconds, and same as server_config.min_lease
	  * Solution: when (lease_time_align == server_config.min_lease), then use server_config.lease as least time.
	  */
