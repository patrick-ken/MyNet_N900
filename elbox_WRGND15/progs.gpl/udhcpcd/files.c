...
//	printf("DHCPD: Get hostname [ %s ]\n",val);
	...
	if (retval)	retval = read_haddr(val, chaddr);
...
static struct config_keyword keywords[] =
{
	...
	{"lan_ip",			read_ip,  &(server_config.lan_ip),			"192.168.1.1"},
...
{
	FILE *in;
	char buffer[103], orig[103], *token, *line;
	int i;
	...
	while (fgets(buffer, 103, in))
	{
		if (strchr(buffer, '\n')) *(strchr(buffer, '\n')) = '\0';
		strncpy(orig, buffer, 103);
