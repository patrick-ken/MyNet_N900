	...
	case '1':
		...
		if (*flags)
			xtables_error(PARAMETER_PROBLEM,
			           "CONNMARK2 target: Can't specify --set-mark twice");
		*flags = 1;
		break;
	case '2':
		...
		if (*flags)
			xtables_error(PARAMETER_PROBLEM,
			           "CONNMARK2 target: Can't specify --save-mark twice");
		*flags = 1;
		break;
	case '3':
		...
		if (*flags)
			xtables_error(PARAMETER_PROBLEM,
			           "CONNMARK2 target: Can't specify --restore-mark twice");
		*flags = 1;
		break;
	case '4':
		if (!*flags)
			xtables_error(PARAMETER_PROBLEM,
			           "CONNMARK2 target: Can't specify --mask without a operation");
                             ...
                             struct xt_entry_target **target)
{
	...
	case '=': /* --set-xmark */
	case '-': /* --set-mark */
		xtables_param_act(XTF_ONE_ACTION, "CONNMARK2", *flags & F_MARK);
		if (!xtables_strtoui(optarg, &end, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNMARK2", "--set-xmark/--set-mark", optarg);
		if (*end == '/')
	...
	case '&': /* --and-mark */
		...
		if (!xtables_strtoui(optarg, NULL, &mask, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNMARK2", "--and-mark", optarg);
		info->mode   = XT_CONNMARK2_SET;
		info->ctmark = 0;
		info->ctmask = ~mask;
		*flags      |= F_MARK;
	...
	case '|': /* --or-mark */
		...
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNMARK2", "--or-mark", optarg);
		info->mode   = XT_CONNMARK2_SET;
		info->ctmark = value;
		info->ctmask = value;
		*flags      |= F_MARK;
	...
	case '^': /* --xor-mark */
		xtables_param_act(XTF_ONE_ACTION, "CONNMARK2", *flags & F_MARK);
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNMARK2", "--xor-mark", optarg);
		info->mode   = XT_CONNMARK2_SET;
		info->ctmark = value;
		info->ctmask = 0;
		*flags      |= F_MARK;
	...
	case 'n': /* --nfmask */
		if (!(*flags & F_SR_MARK))
			xtables_error(PARAMETER_PROBLEM, "CONNMARK2: --save-mark "
			           "or --restore-mark is required for "
			           "--nfmask");
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNMARK2", "--nfmask", optarg);
		info->nfmask = value;
		return true;
	...
	case 'c': /* --ctmask */
		if (!(*flags & F_SR_MARK))
			xtables_error(PARAMETER_PROBLEM, "CONNMARK2: --save-mark "
			           ...
			           "--ctmask");
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNMARK2", "--ctmask", optarg);
		info->ctmask = value;
		return true;
	...
	case 'm': /* --mask */
		if (!(*flags & F_SR_MARK))
			xtables_error(PARAMETER_PROBLEM, "CONNMARK2: --save-mark "
			           ...
			           "--mask");
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNMARK2", "--mask", optarg);
                  ...
                  int numeric)
{
	...
	case XT_CONNMARK2_SET:
		if (info->ctmark == 0)
			printf("CONNMARK2 and 0x%x ",
			       (unsigned int)(u_int32_t)~info->ctmask);
		else if (info->ctmark == info->ctmask)
			printf("CONNMARK2 or 0x%x ", info->ctmark);
		else if (info->ctmask == 0)
			printf("CONNMARK2 xor 0x%x ", info->ctmark);
		else if (info->ctmask == 0xFFFFFFFFU)
			printf("CONNMARK2 set 0x%x ", info->ctmark);
		else
			printf("CONNMARK2 xset 0x%x/0x%x ",
	...
	case XT_CONNMARK2_SAVE:
		if (info->nfmask == UINT32_MAX && info->ctmask == UINT32_MAX)
			...
			printf("CONNMARK2 save mask 0x%x ", info->nfmask);
		else
			printf("CONNMARK2 save nfmask 0x%x ctmask ~0x%x ",
	...
	case XT_CONNMARK2_RESTORE:
		if (info->ctmask == UINT32_MAX && info->nfmask == UINT32_MAX)
			...
			printf("CONNMARK2 restore mask 0x%x ", info->ctmask);
		else
			printf("CONNMARK2 restore ctmask 0x%x nfmask ~0x%x ",
