...
timer_handler(void *data)
{
	...
	if (iface->init_racount < MAX_INITIAL_RTR_ADVERTISEMENTS)
	{
		...
		/* if not exceeded MAX_INITIAL_RTR_ADVERTISEMENTS times,                                                   
			use MAX_INITIAL_RTR_ADVERT_INTERVAL as new timer else                                                    
			keep random timer that between MinRtrAdvInterval and                                                       
			MaxRtrAdvInterval                                                                                          
		*/                                                                                                         
		if (iface->init_racount < MAX_INITIAL_RTR_ADVERTISEMENTS)
...
#else
	/* Do nothing, we don't need privilege. */
...
#else
	/* Just check file state. */
	if ( stat(conf_file, &stbuf) )
		return (-1);
#endif
