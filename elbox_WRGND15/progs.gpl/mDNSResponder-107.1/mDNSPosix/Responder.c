...
$Log: Responder.c,v $
Revision 1.3  2005/07/22 04:24:29  r05549
Send deregister packet when deregisting.

Revision 1.2  2005/07/20 09:45:53  r05549
no message
...
/*static void parse_extra_names(char *names)
{
	...
	for(name = strtok(names , ",") ; name != NULL ; name = strtok(NULL , ","))
	{
		...
		{
			printf(LOC_FMT"too many extra name, if you need more names, modify EXTRA_NAME_MAX_QTY\n" , LOC_ARG);
			break;
		}

		gExtraNames[gExtraNamesQty] = strdup(name);
		++gExtraNamesQty;
	}
}*/
#endif //ELBOX_PROGS_GPL_MDNSRESPONDER_EXTRA_NAME
...
//++++ Stanley add to remember pid 2005.07.20
        FILE *fp;
        int  junk;
...
//---- Stanley
    ...
    int i=0;
    for(i=0;i<gExtraNamesQty;i++)
    {
    	if(gExtraNames[i])//marco
    		free(gExtraNames[i]);
    }
    return result;
}
