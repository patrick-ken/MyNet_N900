...
$Log: mDNSPosix.c,v $
Revision 1.2  2005/08/03 10:14:11  r05549
fix gcc3.3.3 compiler error. The gcc3.3.3 can't distinguish multicast address structure defined by mDNSResponder. Modify it to fix format inet_addr("244.0.0.251"). By the way, the original code is correct in gcc3.3.5.
...
mDNSlocal int SetupSocket(struct sockaddr *intfAddr, mDNSIPPort port, int interfaceIndex, int *sktPtr)
	...
	if (intfAddr->sa_family == AF_INET)
		...
		if (err == 0 && port.NotAnInteger)
			{
//++++ Stanley modify to fix invalid argument error 2005.08.03
...
mStatus mDNSPosixRemoveFDFromEventLoop( int fd)
	...
	for ( iSource=(PosixEventSource*)gEventSources.Head; iSource; iSource = iSource->Next)
		...
		if ( fd == iSource->fd)
			...
			if(iSource->Context)//marco
			{
				free(iSource->Context);
				iSource->Context=0;
			}
			free( iSource);
			DetermineMaxEventFD();
