...
#ifdef ELBOX_PROGS_GPL_MDNSRESPONDER_EXTRA_NAME
/*int get_mac_address(char *ifname , unsigned char *mac_addr)
{
	struct ifreq ifr;
...
{
	int index;
	//domainname extra_names[EXTRA_NAME_QTY];
	NetworkInterfaceInfo *primary = FindFirstAdvertisedInterface(m);

	//build_extra_names(m , set , extra_names);
