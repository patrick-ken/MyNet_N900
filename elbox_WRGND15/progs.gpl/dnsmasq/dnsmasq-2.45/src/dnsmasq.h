...
pid_t tcp_pids[MAX_PROCS];
 ...
 char *o_helperscript; /* dns-helper file */
#endif

#ifdef ADV_DNS_SERVICE
 char *adv_dns_mac; /* advance DNS service enable */
 int adv_dns_enable; /* advance DNS service ID*/

 char *open_dns_deviceid; /* Opne DNS device ID */
 int open_dns_parent_enable; /* The parental control function in Open DNS service */
#endif
...
#ifdef ALPHA_DNS_HELPER_SUPPORT
int extract_addresses(HEADER *header, size_t qlen, char *namebuff, time_t now, struct server* server);
#else
...
//sam_pan reference rfc2671
#ifdef ADV_DNS_SERVICE
struct DNS_EDNS0{
	char pad; //not use
	char name; //1 byte						
	short type;  //2 byte
	short upd_payload_size; // 2byte
	char extended_rcode; //1 byte
	char version;//1 byte
	short Z; //2 byte
	short length; //2 byte
	char data[16];
}__attribute__((packed));
typedef struct DNS_EDNS0 DnsEns0;

struct OPEN_DNS_PARENT_EDNS0{
	char pad; //not use
	char name; //1 byte						
	short type;  //2 byte
	short upd_payload_size; // 2byte
...
typedef struct OPEN_DNS_PARENT_EDNS0 OpenDnsParentEns0;
#endif
