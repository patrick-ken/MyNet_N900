...
#define DUMP_RANGE_REGISTER(REG, IDX)  do {				\
	unsigned int en, lo, hi;					\
	UBICOM32_GET_RANGE_REGISTER(REG, IDX, lo, hi, en);		\
...
#else
#define DUMP_RANGE_REGISTER(REG, IDX)  do {				\
	unsigned int en, uen, lo, hi;					\
	UBICOM32_GET_RANGE_REGISTER(REG, IDX, lo, hi, en, uen);		\
	printk(KERN_NOTICE REG"Range"IDX": en(kern-user):%04x-%04x, range: %08x-%08x\n", \
	       (unsigned int)en,					\
	       (unsigned int)uen,					\
	       (unsigned int)lo,					\
	       (unsigned int)hi);					\
...
asmlinkage void ret_from_fork_to_user(void);
asmlinkage void ret_from_fork_to_kernel(void);
...
void cpu_idle(void)
{
	...
	while (1) {
		...
		/*
		 * Test to make sure that no interrupts are accidentally
		 * enabled.
		 */
...
void show_regs(struct pt_regs *regs)
{
	...
	printk(KERN_NOTICE "ucsr: %08x\n",
		(unsigned int)regs->ucsr);
#endif

#if defined(IP8000)
	printk(KERN_NOTICE "int_mask0: %08x, int_mask1: %08x, int_mask2: %08x\n",
	        (unsigned int)regs->int_mask0,
	        (unsigned int)regs->int_mask1,
		(unsigned int)regs->int_mask2);
#else
	...
	printk(KERN_NOTICE "pgd: %08x, asids: %08x, mask: %08x, cmp: %08x\n",
	       (unsigned int)regs->pgd,
	       (unsigned int)regs->asids,
	       (unsigned int)regs->asid_mask,
	       (unsigned int)regs->asid_cmp);
#endif
	...
	/*
	 * Dump global range registers
	 */
	printk(KERN_NOTICE "Global Range Registers\n");
...
/*
 * sys_fork()
 *	Implement a fork() system call.
 */
asmlinkage int sys_fork(struct pt_regs *regs)
{
#if defined(CONFIG_MMU)
	return do_fork(SIGCHLD, regs->an[7], regs, 0, NULL, NULL);
#else
	return -EINVAL;
#endif
}
...

{
	...
	/*
	 * Copies the pt_regs area from the syscall entry.
	 */
	...
	*childregs = *regs;
	...
	if (usp) {
		 ...
		 */
		childregs->an[7] = usp;
		p->thread.a5 = (unsigned long)ret_from_fork_to_user;
#if (UBICOM32_ARCH_VERSION >= 5)
		childregs->csr |= (1 << UBICOM32_CSR_PREV_PRIV_BIT);
#endif
	} else {
		/*
		 * Kernel thread
		...
		childregs->an[7] = (unsigned long)task_stack_page(p) +
			THREAD_SIZE - 8;
		p->thread.a5 = (unsigned long)ret_from_fork_to_kernel;
#if (UBICOM32_ARCH_VERSION >= 5)
		childregs->csr &= ~(1 << UBICOM32_CSR_PREV_PRIV_BIT);
#endif
	}
 ...
 */
asmlinkage int sys_execve(char *name, const char * const *argv,
