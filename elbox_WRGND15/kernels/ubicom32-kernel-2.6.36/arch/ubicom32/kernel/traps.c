...
{
	int is_pppd=0;
	int is_kernel_mode_page_fault=0;
	...
	if (current) {
		...
		if ( strcmp(current->comm, "pppd" ) == 0 ) is_pppd=1;
	}
	...
	if ( strcmp(str, "Page Fault Terminate" ) == 0 ) is_kernel_mode_page_fault=1;
#endif
	show_regs(regs);
	show_vmas(current);
	...
	// Bouble - 20130322: sometimes, pppd disconnect will cause page fault in ubicom paltform, 
	// workaround: reboot machine
    //if (is_pppd == 1 && is_kernel_mode_page_fault == 1 )
