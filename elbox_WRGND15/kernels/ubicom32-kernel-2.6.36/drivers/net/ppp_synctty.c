		...
		char *flags, int count)
{
	...
	if (p[0] & 1) {
		...
		goto err;
#if defined(ALPHA_IP_ALIGN)
	offset = (unsigned int)skb->data % 4;
	if (offset == 1 || offset == 3) {
		skb_push(skb, 4-offset);
		skb->tail = skb->tail - (4-offset);
		skb->len  = skb->len - (4-offset);
		p = skb->data;
		p += (4-offset);
		memmove(skb->data, p, skb->len + 1);
	}
#endif
