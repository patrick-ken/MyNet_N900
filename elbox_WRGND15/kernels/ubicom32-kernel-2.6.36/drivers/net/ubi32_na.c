...
struct na_desc {
	...
	volatile uint16_t mss;		/* Maximum segment size */
	volatile int16_t interface_num;	/* Interface number */
};

#define NA_DESC_STATUS_PKT 0x0001
					/* Descriptor is for packet data (0x01) or metadata (0x00) */
#define NA_DESC_STATUS_IPV4_IP_CHECKSUM_VALID 0x0002
					/* Descriptor is for packet data where IPv4 header checksum is valid */
#define NA_DESC_STATUS_GEN_IPV4_IP_CHECKSUM 0x0004
					/* Descriptor is for packet data where the IPv4 header checksum should be generated */
#define NA_DESC_STATUS_IP_TRANSPORT_CHECKSUM_VALID 0x0008
					/* Descriptor is for packet data where the IP transport checksum is valid */
#define NA_DESC_STATUS_GEN_IP_TRANSPORT_CHECKSUM 0x0010
					/* Descriptor is for packet data where the IP transport checksum should be generated */
#define NA_DESC_STATUS_FIRST_SEGMENT 0x0020
					/* Descriptor contains the first segment of a frame */
#define NA_DESC_STATUS_LAST_SEGMENT 0x0040
					/* Descriptor contains the last segment of a frame */
#define NA_DESC_STATUS_DISCARD 0x0080
					/* Descriptor is for a packet payload that should be discarded by the NA */
#define NA_DESC_STATUS_SEG_EN 0x0100
					/* Perform segmentation */
#define NA_DESC_STATUS_SEG_TSO 0x0200
					/* Segmentation type is TSO */
#define NA_DESC_STATUS_SEG_UFO 0x0400
					/* Segmentation type is UFO */
#define NA_DESC_STATUS_SEG_TSO6 0x0800
					/* Segmentation type is TSO6 */
#define NA_DESC_STATUS_SEG_VLAN 0x1000
					/* VLAN tag is present */

#define NA_DESC_STATUS_SEG_MASK (NA_DESC_STATUS_SEG_TSO | \
						NA_DESC_STATUS_SEG_UFO | \
						NA_DESC_STATUS_SEG_TSO6 | \
						NA_DESC_STATUS_SEG_VLAN)

#define NA_DESC_STATUS_SEG_SHIFT (9)
...
static void ubi32_na_rx_metadata_ipv4_rule_sync(struct na_ipv4_rule_sync *nirs)
{
	...
	ubi32_na.ipv4_accelerated_tx_bytes += nirs->flow_tx_byte_count + nirs->return_tx_byte_count;//modify origin:nirs->return_tx_packet_count
...
static void ubi32_na_rx_metadata_ipv6_rule_sync(struct na_ipv6_rule_sync *nirs)
{
	...
	ubi32_na.ipv6_accelerated_tx_bytes += nirs->flow_tx_byte_count + nirs->return_tx_byte_count;// Modify origin: nirs->return_tx_packet_count
...
static void ubi32_na_rx_metadata_interface_stats_sync(struct na_interface_stats_sync *niss)
{
	...
	#if 0
	if ((niss->tx_bytes !=0) && (niss->interface == 1))  
	{
		printk("# [TX_bytes]=%llu [RX_bytes]=%llu \n",priv->stats64.tx_bytes,priv->stats64.rx_bytes);
	}
	#endif
	...
	while (count < quota) {
		...
		if (unlikely(!(status & NA_DESC_STATUS_PKT))) {
			...
			if (priv->vlgrp && skb->data[12] == 0x81 && skb->data[13] == 0x00) {
				...
				vlan_gro_receive(&ubi32_na.napi, priv->vlgrp, skb->vlan_tci, skb);
			...
			skb->protocol = eth_type_trans(skb, dev);
			napi_gro_receive(&ubi32_na.napi, skb);
			dev->last_rx = (long)ubi32_na.last_rx_jiffies;
		}
...
static int ubi32_na_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (likely(nr_frags == 0)) {
		...
		if (skb_is_gso(skb)) {
			...
			tx_flags |= NA_DESC_STATUS_SEG_EN;
			if (skb_shinfo(skb)->gso_type & SKB_GSO_TCPV4) {
				tx_flags |= NA_DESC_STATUS_SEG_TSO;
			} else if (skb_shinfo(skb)->gso_type & SKB_GSO_TCPV6) {
				tx_flags |= NA_DESC_STATUS_SEG_TSO6;
			} else if (skb_shinfo(skb)->gso_type & SKB_GSO_UDP) {
				tx_flags |= NA_DESC_STATUS_SEG_UFO;
			} else {
				BUG_ON(1);
			}
		}
		 ...
		 */
		txdesc = &nim->tx_desc[i];
		txdesc->status = NA_DESC_STATUS_PKT | NA_DESC_STATUS_FIRST_SEGMENT | NA_DESC_STATUS_DISCARD | tx_flags;
		if (likely(skb->ip_summed == CHECKSUM_PARTIAL)) {
			txdesc->status |= NA_DESC_STATUS_GEN_IP_TRANSPORT_CHECKSUM;
		}
		txdesc->interface_num = (int8_t)priv->interface_num;
		...
		while (unlikely(j < (nr_frags - 1))) {
			...
			txdesc->status = NA_DESC_STATUS_PKT | NA_DESC_STATUS_DISCARD | tx_flags;
			if (likely(skb->ip_summed == CHECKSUM_PARTIAL)) {
				txdesc->status |= NA_DESC_STATUS_GEN_IP_TRANSPORT_CHECKSUM;
			}
			txdesc->interface_num = (int8_t)priv->interface_num;
		...
		frag = &skb_shinfo(skb)->frags[j];
		txdesc->status = NA_DESC_STATUS_PKT | NA_DESC_STATUS_LAST_SEGMENT | tx_flags;
		if (likely(skb->ip_summed == CHECKSUM_PARTIAL)) {
			txdesc->status |= NA_DESC_STATUS_GEN_IP_TRANSPORT_CHECKSUM;
		}
		txdesc->interface_num = (int8_t)priv->interface_num;
...
static struct rtnl_link_stats64 *ubi32_na_get_stats64(struct net_device *dev, struct rtnl_link_stats64 *s64)
{
	...
	s64->rx_bytes = priv->stats64.rx_bytes & 0xffffffff;  //justin:workaround for WIN 7 IGD bytes counter test
	s64->rx_errors = priv->stats64.rx_errors;
	s64->rx_dropped = priv->stats64.rx_dropped;
	s64->tx_packets = priv->stats64.tx_packets;
	s64->tx_bytes = priv->stats64.tx_bytes & 0xffffffff;  //justin:workaround for WIN 7 IGD bytes counter test
...
static struct net_device *ubi32_na_alloc_netdev(int32_t interface_num, struct na_if_map *nim)
{
	...
	if (interface_num < 2) {
		...
		dev->vlan_features |= NETIF_F_SG | NETIF_F_HW_CSUM | NETIF_F_TSO | NETIF_F_TSO6;
	}
...
static ssize_t ubi32_na_get_wan_status(struct sys_device *dev, struct sysdev_attribute *attr, char *buf)
{
	struct net_device *ldev = ubi32_na.devices[1];
	struct ubi32_na_private *priv = netdev_priv(ldev);
	struct na_if_map_interface *nimi = priv->nimi;
...
static ssize_t ubi32_na_get_wan_txbytes(struct sys_device *dev, struct sysdev_attribute *attr, char *buf)
{
	struct net_device *ldev = ubi32_na.devices[1];
	struct ubi32_na_private *priv = netdev_priv(ldev);

	return sprintf(buf, "%llu\n", priv->stats64.tx_bytes & 0xffffffff);
}

static ssize_t ubi32_na_get_wan_rxbytes(struct sys_device *dev, struct sysdev_attribute *attr, char *buf)
{
        struct net_device *ldev = ubi32_na.devices[1];
        struct ubi32_na_private *priv = netdev_priv(ldev);

	return sprintf(buf, "%llu\n", priv->stats64.rx_bytes & 0xffffffff);
}
...
static SYSDEV_ATTR(wan_status, 0644, ubi32_na_get_wan_status, NULL);
static SYSDEV_ATTR(wan_txbytes, 0644, ubi32_na_get_wan_txbytes, NULL);
static SYSDEV_ATTR(wan_rxbytes, 0644, ubi32_na_get_wan_rxbytes, NULL);
...
int ubi32_na_add_sysfs(void)
{
	...
	result = sysdev_create_file(&ubi32_na_sys_dev, &attr_wan_status);
	if (result) {
		na_warning("Failed to register wan status file %d\n", result);
		goto sysfs_cleanup_11;
	}

        result = sysdev_create_file(&ubi32_na_sys_dev, &attr_wan_txbytes);

        result = sysdev_create_file(&ubi32_na_sys_dev, &attr_wan_rxbytes);
