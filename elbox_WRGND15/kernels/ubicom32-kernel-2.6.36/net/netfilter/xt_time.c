...
//+++joel add for over night sechdule.
#if 1
    if(info->daytime_start >= info->daytime_stop && packet_time < info->daytime_stop)
    {
		//Warning : the day difintion is 1~7 map to Mon ~Sun.
		//weekday is 1-7
		//
		u_int8_t lastday = current_time.weekday - 1;
		//if mon's yesterday is Sun.
		if(lastday==0) lastday = 7;
		if(!(info->weekdays_match & (1 << lastday)))
			return false;
	}
	else
	{
