...
int match(struct nf_conn *i, int condition, int parameter)
{
	...
	switch (condition) {
		case PROTO_NUM:
			if (i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum
					== parameter)
				return 1;
			else
				break;
		...
		case IP:
			if (i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip
					== parameter)
				return 1;
			else
				break;
		...
		case PORT:
			if (ntohs(i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all)
					== parameter)
				return 1;
			else
				break;
...
static int kill_all(struct nf_conn *i, void *data)
{
	...
	if (!(para->port == 0
				...
				&& para->ip == 0
				&& para->status == 0)) {
		if (match(i, PROTO_NUM, para->proto_num)
				&& match(i, IP, para->ip)
				&& match(i, PORT, para->port)
				&& match(i, STATUS, para->status))
...
int nf_conntrack_flush_show_usage(
		char *page,
		char **start,
		off_t off,
		int count,
...
void alpha_conntrack_flush_proc_init(struct net *net)
{
	...
	if (proc_flu) {
		proc_flu->write_proc = nf_conntrack_flush;
