		...
		unsigned long count, void *data)
{
	...
	memset(tmp_buffer, 0x0, 512);
	if (copy_from_user(tmp_buffer, buffer, count)) {
		return count;
	}

	if (!strncmp(tmp_buffer, "0", 1)) {
		g_fast_route = 0;
		printk("Disable Fast Route\n");
		return count;
	}
	else {
		g_fast_route = 1;
		printk("Enable Fast Route\n");
	}

	return count;
}

int alpha_fast_route_dump_status(
		char *page,
		char **start,
		off_t off,
		int count,
		...
		void *data)
{
	...
	if (g_fast_route == 0) {
		ret = sprintf(page, "Fast Router is not activated\n");
	}
	else {
		ret = sprintf(page, "Fast Route activated\n");
	}

	return ret;
}

void alpha_fast_route_proc_init()
{
	struct proc_dir_entry *proc_flu;

	proc_flu = create_proc_entry("alpha_fast_route", 0644, NULL);
	if (proc_flu) {
		proc_flu->write_proc = alpha_fast_route_proc;
		proc_flu->read_proc = alpha_fast_route_dump_status;
		proc_flu->data = NULL;
	}
}
#endif
