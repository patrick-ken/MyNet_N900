 ...
 */
static inline int same_dst(const struct nf_conn *ct, const struct nf_conntrack_tuple *tuple)
{
	const struct nf_conntrack_tuple *t;
 ...
 */
static inline int same_ip(const struct nf_conn *ct, const struct nf_conntrack_tuple *tuple)
{
	const struct nf_conntrack_tuple *t;
 ...
 */
struct nf_conn *find_appropriate_conntrack(struct net *net,
		u16 zone,
		const struct nf_conntrack_tuple *tuple)
{
	unsigned int h = hash_by_src(net, zone, tuple);
	...
	rcu_read_lock();
	hlist_for_each_entry_rcu(nat, n, &net->ipv4.nat_bydest[h], bydest) {
		ct = nat->ct;
...
struct nf_conn *find_appropriate_conntrack_for_fullcone(struct net *net,
		u16 zone,
		const struct nf_conntrack_tuple *tuple)
{
	unsigned int h = hash_by_src(net, zone, tuple);
...
#ifdef CONFIG_NF_DNAT_SHIFT
	if(range->flags & IPT_DNAT_TO_SHIFT) {
		goto SHIFT;
	}
#endif
