...
{	
	...
	SaveXML: function(wlan_uid)
	{
		...
		if (/n/.test(OBJ("wlan_mode"+str_Aband).value) || OBJ("wlan_mode"+str_Aband).value=="ac")
		{
	...
	{
		if (!OBJ("auto_ch"+str_Aband).checked && (/n/.test(OBJ("wlan_mode"+str_Aband).value)||/ac/.test(OBJ("wlan_mode"+str_Aband).value) ) )
		{
...
//			this.OnChangeWLMode(str_Aband);	
	...
	{	
		...
		else if(OBJ("wlan_mode"+str_Aband).value=="ac")
		{
			this.bandWidth	= XG(phywlan+"/media/dot11n/bandwidth");
			COMM_SetSelectValue(OBJ("bw"+str_Aband), this.bandWidth);
			OBJ("bw"+str_Aband).disabled	= false;
