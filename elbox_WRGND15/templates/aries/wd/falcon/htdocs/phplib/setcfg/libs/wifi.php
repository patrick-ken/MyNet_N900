...
function phyinf_setcfg($prefix_phyinf, $minor)
{
	...
	if ($minor==1)
	{
		...
		set($to."/media/freq",				query("media/freq"));
		...
		//beamforming setting
		if(query("media/beamforming/beamformer")!="")
		{
			set($to."/media/beamforming/beamformer",query("media/beamforming/beamformer"));
			set($to."/media/beamforming/beamformee",query("media/beamforming/beamformee"));
		}	
	}
...
function wifi_setcfg($prefix)
{
	foreach($prefix."/phyinf")
	{
		...
		if($uid=="WIFI-STA")  $minor=1;//For wireless station mode in dir-865
