...
function IPTWAN_build_command($name)
{
	...
	if ($path!="")
	{
		...
		$iptcmd2 =  "iptables -t nat -I PRE.".$name;        //Remote management have high priority in Chain:PRE.WAN-1.
		...
		if ($https_rport != "" && $inbf != "denyall" )
		{
			...
			if ($inbf != "")
			{
				...
				fwrite("a",$_GLOBALS["START"], $iptcmd2." -p tcp --dport ".$https_rport." "."-j CK_INBOUND".$inbfn."\n");
			}
			$hostip = query($path."/weballow/hostv4ip");

			if ($hostip != "")
			{
				fwrite("a",$_GLOBALS["START"], $iptcmd2." -s ".$hostip." -p tcp --dport ".$https_rport." -j REDIRECT --to-port 443\n");
			}
			else
			{
				fwrite("a",$_GLOBALS["START"], $iptcmd2." -p tcp --dport ".$https_rport." -j REDIRECT --to-port 443\n");
			}
		...
		else if ($web != "" && $inbf != "denyall")
		{
			...
			if ($hostip != "")
			{
				...
				fwrite("a",$_GLOBALS["START"], $iptcmd2." -s ".$hostip." -p tcp --dport ".$web." -j ACCEPT\n");
			}
			else
			{
				if (query($path."/inbfilter")!="") fwrite("a",$_GLOBALS["START"], $iptcmd." -p tcp --dport ".$web." "."-j CK_INBOUND".$inbfn."\n");
				fwrite("a",$_GLOBALS["START"], $iptcmd2." -p tcp --dport ".$web." -j ACCEPT\n");
			}
	...
	if ($https_rport != "") {
		fwrite("a", $_GLOBALS["STOP"], "xmldbc -P /etc/scripts/RemoveIGDEntry.php -V EXTERNALPORT=".$https_rport." -V DEST=WebMgt \n");
	}
	if ($web != "") {
		fwrite("a", $_GLOBALS["STOP"], "xmldbc -P /etc/scripts/RemoveIGDEntry.php -V EXTERNALPORT=".$web." -V DEST=WebMgt \n");
	}
