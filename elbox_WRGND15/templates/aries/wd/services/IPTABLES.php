...
IPT_newchain($START, "nat", "PRE.PROXYD");
IPT_newchain($START, "nat", "PRE.PROXYD_GUEST");
/*Add by Joseph for Access control and Website filter*/
IPT_newchain($START, "filter", "FOR_POLICY");
/*Add by Argus for Parental control*/
$model         = query("/runtime/device/modelname");
if ($model == "MyNetN600" || $model == "MyNetN750")
{
	IPT_newchain($START, "filter", "FWD.PCTL-1");
	IPT_newchain($START, "filter", "FWD.PCTL-2");
	IPT_newchain($START, "filter", "PCTL.SCH");
	IPT_newchain($START, "filter", "PCTL.DAILY");
	IPT_newchain($START, "filter", "PCTL.GUEST");
	fwrite(a,$START, "iptables -A FWD.PCTL-1 -j PCTL.SCH\n");
	fwrite(a,$START, "iptables -A FWD.PCTL-1 -j PCTL.DAILY\n");
	fwrite(a,$START, "iptables -A FWD.PCTL-1 -j PCTL.GUEST\n");
	fwrite(a,$START, "iptables -A FWD.PCTL-2 -m connmark2 --mark 64/192 -j ACCEPT\n");
	fwrite(a,$START, "iptables -A FWD.PCTL-2 -m connmark2 --mark 128/192 -j DROP\n");
}
...
while ($i<2)
{
	...
	fwrite("a",$START,
		...
		$iptcmd." -p tcp ! --tcp-flags SYN SYN -m state --state NEW -j DROP\n".
