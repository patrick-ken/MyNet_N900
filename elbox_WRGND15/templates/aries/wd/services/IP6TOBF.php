	...
	if ($def_policy != "DISABLE")
	{
		...
		foreach ($firewallp."/entry")
		{
			...
			if ($srcinf!="")
			{
				...
				if(isfile("/proc/sys/net/bridge/bridge-nf-call-ip6tables")!=1)
			...
			*/
			...
			if ($policy == "DROP")	// for LOG when DROP pkts.
			{
				if($prot=="TCP+UDP")
				{
				...
				/*
				else if ($prot == "ICMPv6")		// add ICMPv6, by Jerry Kao.
				{								
					foreach ("dst/type/entry")
					{
						$icmpv6_type = "--icmpv6-type ".query("name");																		
						
						fwrite("a", $_GLOBALS["START"], 
								    'ip6tables -A '.$chain.' --protocol icmpv6 '.$icmpv6_type.' -j LOG --log-level notice --log-prefix DRP:006:\n');						
					}					
				}
				*/				
				else
			...
			{								
				foreach ("dst/type/entry")
				{
					...
					fwrite("a", $_GLOBALS["START"], 
							    'ip6tables -A '.$chain.' --protocol icmpv6 '.$icmpv6_type.' -j '.$policy.'\n');					
				}					
			}
			*/			
			else
			{
				//if ($policy != "DROP")	// add for removing DROP, by Jerry Kao.
				fwrite("a",$_GLOBALS["START"], 'ip6tables -A '.$chain.' -p '.$prot.' '.$IPT.' -j '.$policy.'\n');
			}
...
/*
  service IP6TOBF.php for IPv6, created by Jerry Kao.
*/
...
$CHAIN = 'FWD.OBFILTER';
XNODE_set_var($CHAIN.".USED", "0");		// in value of /runtime/services/globals/var/name=
