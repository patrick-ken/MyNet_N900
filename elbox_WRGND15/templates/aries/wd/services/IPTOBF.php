...
function guest_zone_ip_check($xmlpath,$inf)
{
	foreach($xmlpath."/entry")
	{
		if( query("src/inf") == $inf )
		{
			if(query("dst/host/start") != "" && query("dst/host/start") != "255.255.255.255")
			{
				$lan = XNODE_getpathbytarget("", "inf", "uid", $inf, 0);
				$lan_inet = XNODE_getpathbytarget("/inet", "entry", "uid", query($lan."/inet"), 0);
				set("dst/host/start", query($lan_inet."/ipv4/ipaddr"));
			}
		}
	}
}
...
/* check LAN-2 ipaddr before setting rules */
guest_zone_ip_check("/acl/obfilter2","LAN-2");
