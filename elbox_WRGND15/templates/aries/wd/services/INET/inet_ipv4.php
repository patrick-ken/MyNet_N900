...
function inet_ipv4_static($inf, $ifname, $inet, $inetp, $default)
{
	...
	startcmd("event WANPORT.LINKUP add 'service INTERNET_PING restart'");
	startcmd("event WANPORT.LINKDOWN add 'event WAN-1.DISCONNECTED;xmldbc -s /runtime/device/wan_status 0'");
	...
	{	
		...
		if($linkstatus!="")     						// ensure that WANPORT is plugged
 			startcmd("event WANPORT.LINKUP");
		/* When user change from DHCP mode to Static IP mode in web, we need run it. 20120128 Daniel Chen */
	}

	/* Stop script */
	stopcmd("event WANPORT.LINKUP flush");
...
function inet_ipv4_dynamic($inf, $ifname, $inet, $inetp)
{
	 ...
	 * we might need a autoip before we get an available ip address.
	 */
	$stsp = XNODE_getpathbytarget("/runtime", "inf", "uid", $inf, 0);
	$autoip = query($stsp."/ipalias/ipv4/autoip");
	if ($autoip != "") {
		set($stsp."/inet/ipv4/ipaddr", $autoip);
		set($stsp."/inet/ipv4/mask", "255.255.0.0");
		startcmd("ip addr add ".$autoip."/255.255.0.0 dev br0\n");
		startcmd("service INFSVCS.".$inf." restart\n");
	}
...
//Debug code; same as above code
//		'echo [$0]: $1 interface=$interface ip=$ip subnet=$subnet router=$router lease=$lease domain=$domain scopt=$scope winstype=$winstype wins=$wins sixrd_prefix=$sixrd_prefix sixrd_prefixlen=$sixrd_prefixlen sixrd_msklen=$sixrd_msklen sixrd_bripaddr=$sixrd_bripaddr sstrout=$sstrout clsstrout=$clsstrout srouter=$srouter msclsstrout=$msclsstrout... > /dev/console\n'.
//		'xmldbc -P '.$hlper.' -V ACTION=$1'.
//			' -V INF='.$inf.
//			' -V INET='.$inet.
//			' -V MTU='.$mtu.
...
//			' -V "MSCLSSTROUT=$msclsstrout"'.
//			' -V "SSTROUT=$sstrout"'.
//			' -V "SCOPE=$scope"'.
//			' -V "WINSTYPE=$winstype"'.
//			' -V "WINS=$wins"'.
//			' -V "SIXRDPFX=$sixrd_prefix"'.
//			' -V "SIXRDPLEN=$sixrd_prefixlen"'.
//			' -V "SIXRDMSKLEN=$sixrd_msklen"'.
//			' -V "SIXRDBRIP=$sixrd_bripaddr"'.			
//			' -V "SDEST=$sdest"'.
//			' -V "SSUBNET=$ssubnet"'.
//			' -V "SROUTER=$srouter" > /var/servd/dhcpc_callback.sh\n'.
//		'echo ">>>>>  /var/servd/dhcpc_callback.sh" > /dev/console\n'.
//		'cat /var/servd/dhcpc_callback.sh\n'.
//		'echo "<<<<<  " > /dev/console\n'.
//		'sh -x /var/servd/dhcpc_callback.sh\n'.
	...
	startcmd(
		...
		'udhcpc '.$unicast.'-i '.$ifname.' -H '.$hostname.' -p '.$udhcpc_pid.' -s '.$udhcpc_helper.' '.$dhcpplus_cmd.' &\n'
		);
