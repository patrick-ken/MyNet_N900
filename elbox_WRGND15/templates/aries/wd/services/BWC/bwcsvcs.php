...
function apply_auto_detect_bw($runtime_inf)
{
	include "/htdocs/webinc/feature.php";
	$name = query($runtime_inf."/bwc/uid");
	$auto_bw_flag = query($runtime_inf."/bwc/autobandwidth");

	if($auto_bw_flag != "1")
		return;

	$auto_detect_bw = query($runtime_inf."/auto_detect_bw");
	if($FEATURE_MODEL_NAME=="av"||$FEATURE_MODEL_NAME=="db600")
	{
		if( $auto_detect_bw <= 128 || $auto_detect_bw >= 6000)
		{//follow WD request
			set($runtime_inf."/bwc/bandwidth" , "0" );//For templates.php
			set($runtime_inf."/auto_detect_bw" , "0" );//For adv_qos.php
			set($runtime_inf."/bwc/BandWidthField" , "FAKE" );//set FAKE flag for Web notification
		}
		else
		{
			set($runtime_inf."/bwc/bandwidth" , $auto_detect_bw );
		}	
	}
	else
	{
...
function bwc_tc_spq_start($rtbwcp, $name, $ifname)
{
	...
	{
		/* add prio qdisc. */
		startcmd($tc_qd_add." parent 66:1 handle 20:0 prio bands 8 priomap 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7");
	}
	...
	/* 2. DSCP value 46(0x2e) (EF, Expedited Forwarding) as highest */
	$mark_val = 16384;
	$mark_cmd= " -j CONNMARK --set-xmark ".$mark_val."/16777088";
	startcmd($ipt_out_add_prefix." -m dscp --dscp 0x2e".$mark_cmd);
	if ($prefix == "WAN")	{	$bwc_main_chain_wan_rules++;	}
	else if ($prefix == "LAN")	{	$bwc_main_chain_lan_rules++;	}
	startcmd($ipt_out_add_prefix." -m dscp --dscp 0x2e -j RETURN");
	if ($prefix == "WAN")	{	$bwc_main_chain_wan_rules++;	}
	else if ($prefix == "LAN")	{	$bwc_main_chain_lan_rules++;	}
	...
	foreach($rtbwcp."/rules/entry")
	{
		...
		if (query("enable")=="1" && $entry_idx <= $entry_count )
		{
			...
			$multi_subnet = query($bwcfp."/dst/ipv4/subnet");
			...
			if ($multi_subnet != "")
			{
				...
				if ($prefix == "LAN")
				{
					$in_subnet = cut($multi_subnet, $cut_count, " ");
					while($in_subnet != "")
					{
						$in_subnet = " -s ".$in_subnet;
						startcmd($ipt_out_add_prefix.$in_subnet.$mark_cmd);
						startcmd($ipt_out_add_prefix.$in_subnet." -j RETURN");
						$cut_count++;
						$in_subnet = cut($multi_subnet, $cut_count, " ");
					}
				}
				...
				{
					$out_subnet = cut($multi_subnet, $cut_count, " ");
					while($out_subnet != "")
					{
						$out_subnet = " -d ".$out_subnet;
						startcmd($ipt_out_add_prefix.$out_subnet.$mark_cmd);
						startcmd($ipt_out_add_prefix.$out_subnet." -j RETURN");
						$cut_count++;
						$out_subnet = cut($multi_subnet, $cut_count, " ");
					}
				}
			...
			{			
				...
				if ($proto == "TCP" || $proto == "UDP" || $proto == "ALL" )
	            {
					if ( $dporttype == 1 && $dportname != "" )
					{
						...
						else if ( $dportname == "SKYPE" )
						{
			            	$dportstring=" -m mport --dport 80,443"; 
			            	$in_sportstring=" -m mport --sport 80,443";
						}
						else if ( $dportname == "FACE_TIME" )
						{
							$have_all_proto_flag_apps = 1;
			            	$tcp_dportstring=" -m mport --dport 80,443,4080,5223"; 
			            	$tcp_in_sportstring=" -m mport --sport 80,443,4080,5223";
			            	$udp_dportstring=" -m mport --dport 16399:16472";
			            	$udp_in_sportstring=" -m mport --sport 16399:16472";
						}
						else if ( $dportname == "LOVEFILM" )
						{
							$have_all_proto_flag_apps = 1;
			            	$tcp_dportstring=" -m mport --dport 80,443,5223"; 
			            	$tcp_in_sportstring=" -m mport --sport 80,443,5223";
			            	$udp_dportstring=" -m mport --dport 3478,3479,3658";
			            	$udp_in_sportstring=" -m mport --sport 3478,3479,3658";
						}
						else if ( $dportname == "RHAPSODY" )
						{
							$have_all_proto_flag_apps = 1;
			            	$tcp_dportstring=" -m mport --dport 80,554,4040,7070,8080,443,1755"; 
			            	$tcp_in_sportstring=" -m mport --sport 80,554,4040,7070,8080,443,1755";
			            	$udp_dportstring=" -m mport --dport 1755";
			            	$udp_in_sportstring=" -m mport --sport 1755";
						}
						else if ( $dportname == "BIG_PLANET" )
						{
							$have_all_proto_flag_apps = 1;
			            	$tcp_dportstring=" -m mport --dport 80,443,5223"; 
			            	$tcp_in_sportstring=" -m mport --sport 80,443,5223";
			            	$udp_dportstring=" -m mport --dport 5223,3478,3479,3658";
			            	$udp_in_sportstring=" -m mport --sport 5223,3478,3479,3658";
						}
						else if ( $dportname == "BLOCKBUSTER" )
						{
							$have_all_proto_flag_apps = 1;
			            	$tcp_dportstring=" -m mport --dport 37,443,2190,4430,7287,7288,8000,8080:8089,8101,8102,8200";
			            	$tcp_in_sportstring=" -m mport --sport 37,443,2190,4430,7287,7288,8000,8080:8089,8101,8102,8200";
			            	$udp_dportstring=" -m mport --dport 37,123,2190";
			            	$udp_in_sportstring=" -m mport --sport 37,123,2190";
						}
						else if ( $dportname == "WARCRAFT" )
						{
							$have_all_proto_flag_apps = 1;
			            	$tcp_dportstring=" -m mport --dport 1119:1120,3724,6112";
			            	$tcp_in_sportstring=" -m mport --sport 1119:1120,3724,6112";
			            	$udp_dportstring=" -m mport --dport 1119:1120";
			            	$udp_in_sportstring=" -m mport --sport 1119:1120";
						}
						else if ( $dportname == "XBOX_LIVE" )
						{
							$have_all_proto_flag_apps = 1;
			            	$tcp_dportstring=" -m mport --dport 53,80,3074";
			            	$tcp_in_sportstring=" -m mport --sport 53,80,3074";
			            	$udp_dportstring=" -m mport --dport 53,88,3074";
			            	$udp_in_sportstring=" -m mport --sport 53,88,3074";
						}
						else if ( $dportname == "SPOTIFY" )
						{
			            	$dportstring=" -m mport --dport 80,443,4070";
			            	$in_sportstring=" -m mport --sport 80,443,4070";
						}
						else if ( $dportname == "CinemaNow" )
						{
			            	$dportstring=" -m mport --dport 80,8080";
			            	$in_sportstring=" -m mport --sport 80,8080";
						}
					}
					else
					{ 
			            ...
			            { 
			            	$dportstring=" -m mport --dport ".$dstartport; 
			            	$in_sportstring=" -m mport --sport ".$dstartport; 
			            }
			            ...
			            { 
		 	            ...
		 	            { 
		 	            	$dportstring=" -m mport --dport ".$dstartport.":".$dendport; 
		 	            	$in_sportstring=" -m mport --sport ".$dstartport.":".$dendport; 
		 	            }
		 	            else if ( $dstartport == "" )
		 	            {
		 	            	$skip_this = 1;
				...
				else if ( $have_all_proto_flag_apps == 1 )
				{
					if ($prefix == "LAN")
					{
						startcmd($ipt_out_add_prefix." -p tcp".$in_dipstring.$in_sipstring.$tcp_in_sportstring.$mark_cmd);
						startcmd($ipt_out_add_prefix." -p tcp".$in_dipstring.$in_sipstring.$tcp_in_sportstring." -j RETURN");
						startcmd($ipt_out_add_prefix." -p udp".$in_dipstring.$in_sipstring.$udp_in_sportstring.$mark_cmd);
						startcmd($ipt_out_add_prefix." -p udp".$in_dipstring.$in_sipstring.$udp_in_sportstring." -j RETURN");
					}
					else
					{
						startcmd($ipt_out_add_prefix." -p tcp".$sipstring.$dipstring.$tcp_dportstring.$mark_cmd);
						startcmd($ipt_out_add_prefix." -p tcp".$sipstring.$dipstring.$tcp_dportstring." -j RETURN");
						startcmd($ipt_out_add_prefix." -p udp".$sipstring.$dipstring.$udp_dportstring.$mark_cmd);
						startcmd($ipt_out_add_prefix." -p udp".$sipstring.$dipstring.$udp_dportstring." -j RETURN");
					}
				}
				else
				{
	...
	/* Finally, mark all packets are FROM/TO internet and doesn't have connmark(0) , priority is 1024 */
	$mark_val = 1024;
...
function bwc_tc_wfq_start($rtbwcp, $name, $ifname)
{
	...
	/* trate: total rate (bandwidth) */
	$trate = query($rtbwcp."/bandwidth");
	$auto_bw_flag = query($rtbwcp."/autobandwidth");
	$user_bw_flag = query($rtbwcp."/user_define");
	if ($auto_bw_flag == "1")
	{
		$runtime_node = XNODE_getpathbytarget("/runtime", "inf", "uid", $name, 0);
		$trate = query($runtime_node."/bwc/bandwidth");
	}
	else if ($user_bw_flag == "1")
	{
		$trate = query($rtbwcp."/user_define_bw");
	}
    ...
    /* For WFQ, doesn't set "prio" parameter, when it bet set, then free bandwidth will be whole get by the highest priority class.
      ...
      */
	startcmd($tc_class_add." parent 1:1 classid 1:2 htb rate ".$rate_min_q1.$unit." ceil ".$rate_max_q1.$unit." prio 0");
	startcmd($tc_class_add." parent 1:1 classid 1:3 htb rate ".$rate_min_q2.$unit." ceil ".$rate_max_q2.$unit." prio 0");
	startcmd($tc_class_add." parent 1:1 classid 1:4 htb rate ".$rate_min_q3.$unit." ceil ".$rate_max_q3.$unit." prio 1");
	startcmd($tc_class_add." parent 1:1 classid 1:5 htb rate ".$rate_min_q4.$unit." ceil ".$rate_max_q4.$unit." prio 2");
	...
	/* udp connection have transfer over 4k and packet size > 300 bytes, the disable the flag, use SNAT */
	startcmd($ipt_out_add_prefix." -m connmark --mark 65536/65536 -m connbytes --connbytes 4000: --connbytes-dir both --connbytes-mode bytes -m length --length 300: -j CONNMARK --set-xmark 0/65536");

	/* udp connection have transfer over 10k bytes, disable flag and set priority to skype priority */
	startcmd($ipt_out_add_prefix." -m connmark --mark 65536/65536 -m connbytes --connbytes 10000: --connbytes-dir both --connbytes-mode bytes -j CONNMARK --set-xmark 4096/16777088");

	/* Finally, mark all udp packets are FROM/TO internet and doesn't have connmark(0) , priority is 1024 , but not use SNAT set the flag(0x10000) */
	$mark_val = 66560;
	$mark_cmd= " -p udp -m connmark --mark 0/65408 -j CONNMARK --set-xmark ".$mark_val."/16777088";
	startcmd($ipt_out_add_prefix.$mark_cmd);

    /* Finally, mark all tcp packets are FROM/TO internet and doesn't have connmark(0) , priority is 1024 */
    $mark_val = 1024;
    $mark_cmd= " -m connmark --mark 0/65408 -j CONNMARK --set-xmark ".$mark_val."/65408";
	startcmd($ipt_out_add_prefix.$mark_cmd);
...
function bwc_setup($name)
{
	...
	if(query("/runtime/devdata/countrycode")=="RU" && $bwc_profile_name=="BWC-1")
	{
		if(query($bwcp."/bandwidth")=="RUSSIA")
		{
			set($bwcp."/bandwidth", "1024");
			set($stsp."/bwc/BandWidthField","RUSSIA");
			set($stsp."/bwc/bandwidth","1024");
			set($stsp."/auto_detect_bw" , "1024" );
		}
		else
		{
			set($stsp."/auto_detect_bw" , query($bwcp."/bandwidth") );
			set($stsp."/bwc/BandWidthField","");
		}
	}
	/* apply auto-detect bandwidth if it is necessary (tom, 20111124) */
	apply_auto_detect_bw($stsp);
	...
	else if($bwcp_flag == "TC_WFQ")
	{
		...
		if(query($stsp."/bwc/autobandwidth") == "1")
		{
			if($FEATURE_MODEL_NAME=="av"||$FEATURE_MODEL_NAME=="db600")
			{
				if(query($stsp."/auto_detect_bw") <= 128 || query($stsp."/auto_detect_bw") >= 6000)
				{
					set("/runtime/qos_run" , 0 );
				}
				else
				{
					set("/runtime/qos_run" , 1 );
				}
			}
			else
			{
		...
		if( query($bwcp."/enable") == "1" && query("/runtime/qos_run") != "0")
   	 	{
			if (query("/runtime/qos_run") == "1")
			{
				startcmd("service BWC.LAN-1 restart");
			}
			
			// Stop ralink HW_NAT 
			startcmd("service HW_NAT restart");
			...
			/* it is unnecessary for WD project with ram size 128MB by siyou.
	...
	else if($bwcp_flag == "STREAMENGINE")
	{
		startcmd("service STREAMENGINE restart");
		stopcmd("service STREAMENGINE stop");
	}
	else
	{
