...
function is_guestzone($name)
{
	$path = XNODE_getpathbytarget("", "inf", "uid", $name, 0);
	$name = query($path."/name");
	if($name != "" && strstr($name, "IPv4 Guest Access") != "")
	{
		return 1;
	}
	return 0;
...
function lan_default($infp, $stsp, $name, $dev)
{
	...
	if (query("/proxyd/enable")=="1")
	{
		if (is_guestzone($name) != 1)
		{
			startcmd("iptables -t nat -A PREROUTING -i ".$dev." -p tcp ! -d ".$ipaddr." --dport 80 -j PRE.PROXYD");
			//startcmd("iptables -t nat -A PREROUTING -i ".$dev." -p tcp ! -d ".$ipaddr." --dport 443 -j PRE.PROXYD");
		}
		else
		{
			$path = XNODE_getpathbytarget("", "inf", "uid", $name, 0);
			...
			$ipaddr = query($path."/ipv4/ipaddr");
			startcmd("iptables -t nat -A PREROUTING -i br1 -p tcp ! -d ".$ipaddr." --dport 80 -j PRE.PROXYD_GUEST");
			startcmd("iptables -I INP.OBFILTER 1 -p TCP -i br1 -d ".$ipaddr." -m mport --dports 5450 -j ACCEPT");
			startcmd("iptables -I INP.OBFILTER 1 -p TCP -i br1 -d ".$ipaddr." -m mport --dports 80 -j ACCEPT");
		}	
		//startcmd("iptables -t nat -A PREROUTING -i ".$dev." -p tcp ! -d ".$ipaddr." --dport 8080 -j PRE.PROXYD");
		//startcmd("iptables -t nat -A PREROUTING -i ".$dev." -p tcp ! -d ".$ipaddr." --dport 3128 -j PRE.PROXYD");
	}
...
//marco, for cone	
	...
	/* For Parental Control when ntp disable
	if (query("/device/time/ntp/enable")!="1")
	{
		startcmd("iptables -A FORWARD -j FWD.PCTL-1");
		startcmd("iptables -A FORWARD -j FWD.PCTL-2");
	}*/
...
if ($layout == "router")
{
	...
	{
		startcmd("rmmod nf_conntrack_ipsec_pass");
	}
	...
	{
		startcmd("rmmod nf_nat_sip");
		startcmd("rmmod nf_conntrack_sip");
	}

	if($alg_pptp=="1")
	{
		startcmd("iptables -t filter -I FORWARD 1 -p tcp --dport 1723 -j ACCEPT");
	}
	else
	{
		startcmd("iptables -t filter -I FORWARD 1 -p tcp --dport 1723 -j DROP");
	}
	//marco, for anti-spoof
	$anti_spoof=query("/acl/anti_spoof/enable");
	if ($anti_spoof == "1") startcmd("echo 1 > /proc/sys/net/ipv4/conf/all/rp_filter\n");
	else startcmd("echo 0 > /proc/sys/net/ipv4/conf/all/rp_filter\n");
}
else if ($layout == "bridge")
{
	...
	startcmd("echo 0 > /proc/sys/net/ipv4/conf/all/rp_filter\n");//marco
}
