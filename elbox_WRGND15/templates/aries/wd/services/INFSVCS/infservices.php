...
function infsvcs_setup($name, $cfg, $sts, $ipv6)
{
	...
	if ($orion>0 && $layout=="router")
	{
		startcmd("service ORION start");
	}
	
	if (""!=$bwc) startcmd("service BWC.".$name." restart");

	if( $name == "WAN-1" )
		startcmd("service INTERNET_PING restart");
	else
		startcmd("event ".$name.".CONNECTED");

	if( $name == "BRIDGE-1") startcmd("service INTERNET_PING restart");
	...
	if ($orion>0)		stopcmd("service ORION stop");
...
function infsvcs_wan($index)
{
	...
	/* Trigger uplink bandwidth detection program to find out the bandwidth */
	startcmd("model_name=\`cat /etc/config/image_sign\`");	// Get model name
	startcmd("if [ \$model_name == \"wrgnd16_wd_db600\" ]||[ \$model_name == \"wrgnd13_wd_av\" ];then");	// for av/db600
	startcmd("bw_detect ".$uid." &");
	startcmd("fi");
	...
	if ($lower=="")
	{
		...
		else
		{
			...
			stopcmd("service PARENTCTRL stop");
		}
	...
	if ($upper=="")
	{
		...
		else
		{
			...
			startcmd("service PARENTCTRL restart\n");
		}
			startcmd("service DEVICE.TIME restart");
...
function infsvcs_lan($index)
{
	...
	{
		stopcmd("service IP6TDEFCHAIN restart");
		stopcmd("service IP6TOBF restart");		// add by Jerry Kao.
		stopcmd("service ROUTE6.STATIC restart");
		stopcmd("service ROUTE6.DYNAMIC restart");
		stopcmd("service MULTICAST restart");
	}
	...
	if ($ipv6>0)
	{
		startcmd("service ROUTE6.STATIC restart");
		startcmd("service ROUTE6.DYNAMIC restart");
		startcmd("service IP6TOBF restart");		// add by Jerry Kao.
		startcmd("service IP6TDEFCHAIN restart");
	}
	else
	{
	...
	startcmd("service FTP restart");	//michael_lee
	startcmd("service ITUNES restart");	//michael_lee
	/* ReStart SAMBA and DLNA */
	startcmd("service SAMBA restart");
	$dlna_1 = query("/wd/storage/dlna/USB1");
	$dlna_2 = query("/wd/storage/dlna/USB2");
	if ( $dlna_1 == "1" || $dlna_2 == "1" )
	{
		startcmd("xmldbc -t \"dlna:10:service DLNA restart\"");
	}
...
function infsvcs_bridge($index)
{
	...
	startcmd("if [ \$model_name == \"wrgnd15_wd_pro\" ];then");	// Set number of ports
    startcmd("  num=7");
	startcmd("elif [ \$model_name == \"wrgnd14_wd_storage\" ];then"); 
	startcmd("  num=4");
	startcmd("else");
	startcmd("  num=0");	// for av/db600
	startcmd("fi");

	startcmd("if [ \$num -eq 0 ];then");	// Don't check port status
	startcmd("  event INET.CONNECTED");
	startcmd("else");		// Check that one port is plugged at least
	startcmd("  while [ \$num -ge 0 ];do");
    startcmd("    if [ \"\`psts -i \$num\`\" != \"\" ];then");
    startcmd("      event INET.CONNECTED;break");
    startcmd("    fi");
    startcmd("    let num=\$num-1");
	startcmd("  done");
	startcmd("fi");
	...
	$ftp_1 = query("/wd/storage/ftp/USB1");
	$ftp_2 = query("/wd/storage/ftp/USB2");
	if ( $ftp_1 == "1" || $ftp_2 == "1" )
	{
		startcmd("xmldbc -t \"dlna:20:service FTP restart\"");
	}
}
?>
