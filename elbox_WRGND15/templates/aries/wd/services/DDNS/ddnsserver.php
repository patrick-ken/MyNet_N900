...
function ddns4generate($inf, $phyinf, $ddnsp,$v6inf,$v6enable,$v6phyinf)
{
	...
	if($provider == "TZO")
	{//For TZO Certification tests
		setattr($stsp."/ddns4/execcmd" , "get" ,"urlget get http://wd.echo.tzo.com/ip.shtml | scut -p IPAddress:");
		$ipaddr = query($stsp."/ddns4/execcmd");
		TRACE_debug("Get $ipaddr from TZO server: ".$ipaddr);
		del($stsp."/ddns4/execcmd");
		$version = query("/runtime/device/firmwareversion");
		$useragent	= '"'.$vendor.' '.$model.'/v'.$version.'"';
	...
	if($provider == "TZO")
	{
		$old_ipaddr = query($stsp."/ddns4/ipaddr");
		$old_status = query($stsp."/ddns4/status");
		$old_result = query($stsp."/ddns4/result");
		TRACE_debug("$old_ipaddr is: ".$old_ipaddr);
		TRACE_debug("$old_status is: ".$old_status);
		TRACE_debug("$old_result is: ".$old_result);
		if($ipaddr==$old_ipaddr && $old_status=="IDLE")
		{
			if($old_result=="NONE" || $old_result=="SUCCESS")
			{
				set($stsp."/ddns4/status","IDLE");
				set($stsp."/ddns4/result","NONE");
				TRACE_debug("TZO ip duplicated happened.");
				ddns_error("8");
				return;
			}
		}
