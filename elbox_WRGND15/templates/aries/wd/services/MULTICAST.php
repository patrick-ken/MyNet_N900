...
//Enable ipv6 mld in AP mode
else if ($layout=="bridge" && $mldproxy=="1")
{
	fwrite("a", $START, "mldproxy -c ".$mldcfile." -s ".$mldsfile." &\n");
	fwrite("a",$STOP, 
			'killall mldproxy\n'. 
    ...
    if ($phyinf=="") break;
	$rinfp = XNODE_getpathbytarget("/runtime", "inf", "uid", $ifname, 0);
    $addrtype = query($rinfp."/devnam");
    if($addrtype=="br0")
	{
		fwrite("a", $mldcfile, $phyinf." downstream 1 0\n");
	}
	if($iproxy != "1")
		{
    	fwrite("a", $START, 'echo "enable_switch_snoop" > /proc/alpha/multicast_br0\n');
...
else if ($layout=="bridge" && $iproxy=="1")
{
	fwrite("a", $START, "igmpproxy -c ".$cfile." -s ".$sfile." &\n");
	fwrite("a",$STOP,
			'killall igmpproxy\n'.
