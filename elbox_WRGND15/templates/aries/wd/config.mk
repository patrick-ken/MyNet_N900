# cutomized config for WD
if [ "$(ELBOX_MODEL_NAME)" == "pro" ]; then				\
	SVNREV := $(shell svn info $(TOPDIR)/boards/$(ELBOX_BOARD_NAME)/ubicom | grep Revision: | cut -f2 -d' ');	\
elif [ "$(ELBOX_MODEL_NAME)" == "storage" ]; then				\
	SVNREV := $(shell svn info $(TOPDIR)/boards/$(ELBOX_BOARD_NAME)/ubicom | grep Revision: | cut -f2 -d' ');	\
elif [ "$(ELBOX_MODEL_NAME)" == "dolphin" ]; then                               \
        SVNREV := $(shell svn info $(TOPDIR)/boards/$(ELBOX_BOARD_NAME)/ubicom | grep Revision: | cut -f2 -d' ');       \
else											\
...
if [ "$(ELBOX_MODEL_NAME)" == "db600" ]; then				\
	PREDEFINE_RELIMAGE:=$(shell echo My_Net_N600_$(ELBOX_FIRMWARE_VERSION)_$(BUILDNO));	\
elif [ "$(ELBOX_MODEL_NAME)" == "av" ]; then				\
	PREDEFINE_RELIMAGE:=$(shell echo My_Net_N750_$(ELBOX_FIRMWARE_VERSION)_$(BUILDNO));	\
elif [ "$(ELBOX_MODEL_NAME)" == "pro" ]; then				\
	PREDEFINE_RELIMAGE:=$(shell echo My_Net_N900_$(ELBOX_FIRMWARE_VERSION)_$(BUILDNO));	\
elif [ "$(ELBOX_MODEL_NAME)" == "storage" ]; then				\
	PREDEFINE_RELIMAGE:=$(shell echo My_Net_N900_Central_$(ELBOX_FIRMWARE_VERSION)_$(BUILDNO));	\
elif [ "$(ELBOX_MODEL_NAME)" == "dolphin" ]; then                               \
        PREDEFINE_RELIMAGE:=$(shell echo My_Net_AC1800_$(ELBOX_FIRMWARE_VERSION)_$(BUILDNO));     \
else											\
...
# PREDEFINE_RELIMAGE:=$(shell echo $(ELBOX_MODEL_NAME)_v$(ELBOX_FIRMWARE_VERSION)_$(BUILDNO))
PREDEFINE_UBOOTIMAGE:=$(shell echo $(ELBOX_MODEL_NAME)_uboot_v$(SVNREV)_$(BUILDNO))
endif
