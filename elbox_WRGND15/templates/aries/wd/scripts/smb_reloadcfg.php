...
if ($active!="1")
{
	echo "SAMBA: server is disabled !\" > /dev/console\n";
}
else
{
	echo "if [ ! -d ".$SAMBAP." ]; then mkdir -p ".$SAMBAP."; fi\n";
	if($anonymous_account == 1)
	{
		echo "if [ ! -f ".$SAMBAP."/smbpasswd ]; then adduser nobody; smbpasswd -a nobody -n; fi\n";
	}
	else
	{	
		echo "if [ ! -f ".$SAMBAP."/smbpasswd ]; then ( echo \"".$UserPassword."\"; echo \"".$UserPassword."\" ) | smbpasswd -s -a ".$UserName."; fi\n";
	}
	...
	/*sent to smbd, nmbd to force daemon to reload smb.conf configuration file.*/
	echo "smbcontrol smbd reload-config\n";
	echo "smbcontrol nmbd reload-config\n";
}
?>
