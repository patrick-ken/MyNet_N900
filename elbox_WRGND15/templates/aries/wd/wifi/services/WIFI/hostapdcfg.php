...
function generate_configs($phyinfuid, $output)
{
	...
	/* for WPS icon, add by Jerry Kao */
	$serial_number = query("/runtime/device/serial_number");
	...
	fwrite("a", $output,
		...
		'wps_model_url='.$producturl.'\n'.		// wps_model_url, added by Jerry_Kao.
...
function inf_exist($inf)
{
	$command = "ifconfig |grep ".$inf."";
	setattr("/runtime/hostapd/run", "get", $command);
	$res = get("x", "/runtime/hostapd/run");
	del("/runtime/hostapd/run");
	if ($res == "")
	{
		return 0;
	}
...
foreach ("/runtime/phyinf")
{
	...
	if (query("valid")!="1") continue;
	/* generate the radio list for topology file */
	$uid = query("uid");
	$dev = devname($uid);
	if (inf_exist($dev)!="1") continue;
