...
Page.prototype =
{
	services: "WIFI.PHYINF,DEVICE.ACCOUNT,INET.WAN-1,RUNTIME.INF.WAN-1,RUNTIME.PHYINF,DHCPS4.LAN-1,DHCPS4.LAN-2,RUNTIME.INF.LAN-1,RUNTIME.INF.LAN-2,PARENTCTRL,STORAGE",
	...
	no_usb2 : <? if (query("/runtime/wd/USB1/entry#") > 0 && query("/runtime/wd/USB2/entry#") > 0 ) {echo "0";} else {echo "1";} ?>,
	InitDeviceMode: function ()/* For bridge mode 20120127 Daniel Chen */
	{
		var device_mode = '<? echo $layout;?>';
		OBJ("status_1_bt").style.display = "";
		OBJ("status_1_bt2").style.display = "none";
		OBJ("status_2_bt").style.display = "";
		OBJ("status_2_bt2").style.display = "none";
		if(device_mode != "router")
		{
			//OBJ("row_1").style.display = "none";
			//OBJ("row_2").style.display = "none";
			//OBJ("row_3").style.display = "none";
			OBJ("status_3").style.display = "none";

			OBJ("status_1").innerHTML="";
			OBJ("u_st_networkstatus").innerHTML="";
			OBJ("status_1_bt").style.display = "none";
			OBJ("status_1_bt2").style.display = "none";
			OBJ("status_2").innerHTML=""; 
			OBJ("u_device_number").innerHTML="";
			OBJ("status_2_bt").style.display = "none";
			OBJ("status_2_bt2").style.display = "none";
	...
	InitParentalControl: function ()
	{
		...
		if (PAGE.no_usb2)
		{
			OBJ("u_parental_control_value").innerHTML = XG(this.parent_ctrl+"/security/active")=="1" ? "<? echo I18N("h", "<Enabled>"); ?>" : "<? echo I18N("h", "<Disabled>"); ?>";
		}
		else
		{
			OBJ("d_parental_control_value").innerHTML = XG(this.parent_ctrl+"/security/active")=="1" ? "<? echo I18N("h", "<Enabled>"); ?>" : "<? echo I18N("h", "<Disabled>"); ?>";
		}
		return true;
	},
	...
	InitWLAN: function ( wlan_uid, wifi_module )
	{
		...
		else
		{
            ...
            OBJ("gz_key").innerHTML = "<? echo I18N("h", "<Disabled>"); ?>";
			return true;
		}		
		//guestzone info
		...
		if (COMM_ToBOOL(XG(this.phyinf+"/active")))
		{
			if (!this.guest_zone)
			{
                ...
                OBJ("gz_key").innerHTML = control_str_len(this.GetSecKey(wifi_path));
			}
		}
		else
		{
