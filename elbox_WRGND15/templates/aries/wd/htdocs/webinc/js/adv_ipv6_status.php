 ...
 */
div.textinput_ipv6_st
{
	clear: both;
	color: white;
	height: 24px;
}
div.textinput_ipv6_st span.name
{
	width: 35%;
	float: left;
	text-align: left;
...
div.textinput_ipv6_st span.delimiter
{
	width: 3%;
	float: left;
	text-align: center;
...
div.textinput_ipv6_st span.value
{
	width: 61%;
	float: left;
	text-align: left;
...
Page.prototype =
{
	...
	InitValue: function(xml)
	{
		...
		var svc = PXML.FindModule("DHCPS6.LAN-4");//this is refrenced by adv_ipv6.php
		var rdnss = PXML.FindModule("DEVICE.RDNSS");//this is refrenced by adv_ipv6.php
		var dhcps6 = GPBT(svc+"/dhcps6", "entry", "uid", "DHCPS6-1", false);//this is refrenced by adv_ipv6.php
		//PXML.doc.dbgdump();
<?
...
?>
		{
			...
			OBJ("ipv6_client").style.display = "block";
		}
		else
		{
		...
		if(XG(dhcps6+"/mode")=="STATELESS" && XG(rdnss+"/device/rdnss")!="1" && OBJ("ll_type").innerHTML!="Link-Local")
		{ 
			OBJ("ipv6_client").style.display = "none";
		}
		else
	...
	InitWAN: function ()
	{
		...
		if(str_wanipaddr=="")
		{
			OBJ("wan_address").innerHTML  = "<?echo I18N("j","None");?>";
	...
	InitLAN: function()
	{
		...
		*/
		...
		{
			OBJ("lan_address").innerHTML = "<?echo I18N("j","None");?>";
