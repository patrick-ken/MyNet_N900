...
Page.prototype =
{
	services: "DEVICE.LAYOUT,PHYINF.WAN-1,INET.BRIDGE-1,INET.INF,WAN,RUNTIME.INF.WAN-3,INET.LAN-4,RUNTIME.INF.WAN-1,RUNTIME.INF.WAN-4,DHCPC4.WAN,REBOOT",
	OnLoad: function() {},
	OnUnload: function() {},
	dhcpc4: null,
	OnSubmitCallback: function (code, result)
	{
		...
		case "OK":
			...
			if(COMM_Equal(OBJ("ipv4_macaddr").getAttribute("modified"), true) || COMM_Equal(OBJ("ppp4_macaddr").getAttribute("modified"), true))
			{
				...
				BODY.ShowCountdown("<?echo I18N("h", "Rebooting");?>...", msgArray, this.bootuptime, "http://<?echo $_SERVER['HTTP_HOST'];?>/wan.php");
			}	
			else
			{
				BODY.OnReload();
	...
	PreSubmit: function()
	{
		...
		{
			BODY.ShowAlert("<?echo I18N("j", "The MTU value is invalid.");?>");
			return null;
		}
		...
		if (COMM_Equal(OBJ(mac_obj).getAttribute("modified"), true))
		{
			...
			//----transform MAC address type------------------------------
			var m=OBJ(mac_obj).value;
			if(m.length==12){
				var mac_tmp = m[0]+m[1]+":"+m[2]+m[3]+":"+m[4]+m[5]+":"+m[6]+m[7]+":"+m[8]+m[9]+":"+m[10]+m[11];
				m=mac_tmp;	
			}
			else if (m.search("-") != -1)		{
				var cor=0;
				...
				if(cor==5) {
					var mac_tmp=m;
					mac_tmp=m[0]+m[1]+":"+m[3]+m[4]+":"+m[6]+m[7]+":"+m[9]+m[10]+":"+m[12]+m[13]+":"+m[15]+m[16];
					m=mac_tmp;		
				}
			}
			OBJ(mac_obj).value=m;
	...
	InitPpp4Value: function()
	{
		...
		OBJ("pptp_alwayson").checked		= true;
		...
		OBJ("l2tp_alwayson").checked		= true;
		...
		case "pptp":
			...
			var dialup = XG(this.wan1.inetp+"/ppp4/dialup/mode");
			if		(dialup === "ondemand")	OBJ("pptp_ondemand").checked= true;
			else if	(dialup === "manual")	OBJ("pptp_manual").checked = true;
			else							OBJ("pptp_alwayson").checked = true;
			OBJ("pptp_max_idle_time").value	= XG(this.wan1.inetp+"/ppp4/dialup/idletimeout");
			var dnscount = XG(this.wan1.inetp+"/ppp4/dns/count");
			if (dnscount > 0)	OBJ("pptp_dns1").value	= XG(this.wan1.inetp+"/ppp4/dns/entry:1");
			if (dnscount > 1)	OBJ("pptp_dns2").value	= XG(this.wan1.inetp+"/ppp4/dns/entry:2");
		...
		case "l2tp":
			...
			var dialup = XG(this.wan1.inetp+"/ppp4/dialup/mode");
			if		(dialup === "ondemand")	OBJ("l2tp_ondemand").checked= true;
			else if	(dialup === "manual")	OBJ("l2tp_manual").checked = true;
			else							OBJ("l2tp_alwayson").checked = true;
			OBJ("l2tp_max_idle_time").value	= XG(this.wan1.inetp+"/ppp4/dialup/idletimeout");
			var dnscount = XG(this.wan1.inetp+"/ppp4/dns/count");
			if (dnscount > 0)	OBJ("l2tp_dns1").value	= XG(this.wan1.inetp+"/ppp4/dns/entry:1");
			if (dnscount > 1)	OBJ("l2tp_dns2").value	= XG(this.wan1.inetp+"/ppp4/dns/entry:2");
	...
	PrePppoe: function()
	{
		...
		{
			BODY.ShowAlert("<?echo I18N("j", "The password is mismatched.");?>");
			return null;
		}
		var wan1addrtype = XG(this.wan1.inetp+"/addrtype");
		var over = XG(this.wan1.inetp+"/ppp4/over");
		...
		{
			BODY.ShowAlert("<?echo I18N("j", "Invalid value for idle timeout");?>");
			return null;
		}
	...
	PrePptp: function(type)
	{
		...
		if(type == "russia")	//normal pptp
		{
			...
			XS(this.wan1.infp+"/lowerlayer", "WAN-2");//JERRY
		}
		else
		{
			XS(this.wan1.infp+"/defaultroute", "100");
		...
		if (OBJ("pptp_static").checked)
		{
			...
			XS(this.wan1.inetp+"/ppp4/dns/entry:1", OBJ("pptp_dns1").value);
			cnt++;
			if (OBJ("pptp_dns2").value !== "")
			{
				XS(this.wan2.inetp+"/ipv4/dns/entry:2", OBJ("pptp_dns2").value);
				XS(this.wan1.inetp+"/ppp4/dns/entry:2", OBJ("pptp_dns2").value);
				cnt++;
			}
			XS(this.wan2.inetp+"/ipv4/dns/count", cnt);
			XS(this.wan1.inetp+"/ppp4/dns/count", cnt);
	...
	PreL2tp: function(type)
	{
		...
		if (OBJ("l2tp_static").checked)
		{
			...
			if (OBJ("l2tp_dns1").value === "")	{ BODY.ShowAlert("<?echo I18N("j", "Invalid Primary DNS address");?>"); return null; }
			XS(this.wan2.inetp+"/ipv4/dns/entry:1", OBJ("l2tp_dns1").value);
			XS(this.wan1.inetp+"/ppp4/dns/entry:1", OBJ("l2tp_dns1").value);
			cnt++;
			if (OBJ("l2tp_dns2").value != "")
			{
				XS(this.wan2.inetp+"/ipv4/dns/entry:2", OBJ("l2tp_dns2").value);				
				XS(this.wan1.inetp+"/ppp4/dns/entry:2", OBJ("l2tp_dns2").value);
				cnt++;
			}
			XS(this.wan2.inetp+"/ipv4/dns/count", cnt);			
			XS(this.wan1.inetp+"/ppp4/dns/count", cnt);			
		...
		{
			XS(this.wan2.inetp+"/ipv4/static", "0");
			if (OBJ("l2tp_dns1").value != "")	{ XS(this.wan1.inetp+"/ppp4/dns/entry:1", OBJ("l2tp_dns1").value); cnt++; }
			else XS(this.wan1.inetp+"/ppp4/dns/entry:1", "");
			if (OBJ("l2tp_dns2").value != "")	{ XS(this.wan1.inetp+"/ppp4/dns/entry:2", OBJ("l2tp_dns2").value); cnt++; }
			XS(this.wan1.inetp+"/ppp4/dns/count", cnt);
	...
	OnClickPppoeAddrType: function()
	{
		...
		OBJ("pppoe_ip_address").style.display = OBJ("pppoe_dynamic").checked ? "none" : "block";
	},
	...
	{
		var dis = OBJ("pptp_dynamic").checked ? "none" : "block";
		//OBJ("pptp_ipaddr").disabled	= dis;
		//OBJ("pptp_mask").disabled	= dis;
		//OBJ("pptp_gw").disabled		= dis;
		OBJ("pptp_ip_address").style.display = dis;
		OBJ("pptp_subnet_mask").style.display = dis;
		OBJ("pptp_gateway_ip").style.display = dis;
	},
	...
	{
		var dis = OBJ("l2tp_dynamic").checked ? "none" : "block";
		//OBJ("l2tp_ipaddr").disabled	= dis;
		//OBJ("l2tp_mask").disabled	= dis;
		//OBJ("l2tp_gw").disabled		= dis;
		OBJ("l2tp_ip_address").style.display = dis;
		OBJ("l2tp_subnet_mask").style.display = dis;
		OBJ("l2tp_gateway_ip").style.display = dis;		
	},
	...
	OnClickRPppoeAddrType: function()
	{
		...
		if(XG(this.wan1.infp+"/open_dns/type")==="")
		{
	...
	DisableDNS: function()
	{
		...
		OBJ("ipv4_dns1").title = OBJ("ipv4_dns2").title = "<?echo I18N("h", "Locked by parental control");?>";		
		OBJ("pppoe_dns1").title = OBJ("pppoe_dns2").title = "<?echo I18N("h", "Locked by parental control");?>";		
		OBJ("pptp_dns1").title = OBJ("pptp_dns2").title = "<?echo I18N("h", "Locked by parental control");?>";		
		OBJ("l2tp_dns1").title = OBJ("l2tp_dns2").title = "<?echo I18N("h", "Locked by parental control");?>";		
		OBJ("rpppoe_dns1").title = OBJ("rpppoe_dns2").title = "<?echo I18N("h", "Locked by parental control");?>";			
