...
Page.prototype =
{
	...
	OnClick24Hours: function()
	{
		...
		if (!checked) this.OnChangeTimeFormat(OBJ("timeformat").value);
	},
	...
	OnClickSchSubmit: function()
	{
		...
		{
			alert("<?echo I18N("j", "The prefix or postfix of the name can not be blank.");?>");
			return;
		}
		...
		{
			BODY.ShowAlert("<?echo I18N("j", "Invalid Schedule. The maximum number of permitted schedule rules has been exceeded.");?>");
			return null;
		}		
		...
		for(var i=0; i < this.cfg.length; i+=1)
		{
			if (e.desc === this.cfg[i].desc)
			{
				...
				{
					alert("<?echo I18N("j", "The schedule name can not be the same.");?>");
					return;
				}
			}
		}
		...
		{
			alert("<?echo I18N("j", "No day is selected.");?>");
			return;
		}
		...
		{
			alert("<?echo I18N("j", "The schedule time is not valid.");?>");
			return;
		}
		...
		//check same rule exist or not
			for (j=0; j<this.cfg.length; j++)
			{
				...
				{
					alert('<?echo I18N("j", "The Rule ");?>"'+dsc+'\"<?echo I18N("j", " already exists !");?>');
					return;
				}
			}
			if(update > -1) alert("<?echo I18N("j", "You must reboot after update.");?>");
	...
	{
		this.edit = null;
		OBJ("schsubmit").value = "<?echo I18N("h", "Add");?>";
	...
	OnEdit: function(i)
	{
		...
		OBJ("schsubmit").value = "<?echo I18N("h", "Update");?>";
