...
div.textinput span.value_ipv6
{
    ...
    <? 
		if($lang=="ru"||$lang=="cs")
		echo "left: 380px;";
		else if($lang=="it"||$lang=="no"||$lang=="ja")
		echo "left: 300px;";
		else if($lang=="sv"||$lang=="hu")
		echo "left: 330px;";
		else if($lang=="en"||$lang=="tr"||$lang=="ko")
		echo "left: 280px;";
		else if($lang=="zhtw"||$lang=="zhcn")
		echo "left: 220px;";		
		else echo "left: 350px;";
    ?>
}
div.textinput_dns td
{
  height: 35px;
	line-height: 35px;
	*line-height: 17px;/*For IE 7*/
}
div.textinput_dns td.name_dns
{
	color: white;
	margin-top: 4px;
	text-align: left;
...
Page.prototype =
{
	...
	InitValue: function(xml)
	{
		...
		//New PPPoE session is default for WD.
		COMM_SetRadioValue("pppoe_sess_type", "new");
	...
	*********************************************************/
	...
	OnChangewan_ipv6_mode: function()
	{
		...
		switch(OBJ("wan_ipv6_mode").value)
		{
			...
			//case "RA":
				...
				//OBJ("w_dhcp_dns_auto").checked	= true;
				OBJ("w_dhcp_pdns").disabled = OBJ("w_dhcp_sdns").disabled = COMM_GetRadioValue("w_dhcp_dns_rad")=="auto"?true:false;
				OBJ("ra_lifetime").value = "";
				OBJ("ip_lifetime").value = "";
				OBJ("l_span").style.display = "";
				break;
			...
			case "PPPOE":
                ...
                if((COMM_GetRadioValue("pppoe_sess_type")=="share"?true:false) && over=="eth")
					this.LoadPpp4Value();
				break;
			...
			case "6RD":
				...
				OBJ("ra_lifetime").value = "";
				OBJ("ip_lifetime").value = "";
				OBJ("w_6rd_prefix_1").disabled = OBJ("w_6rd_prefix_2").disabled = OBJ("w_6rd_v4addr_mask").disabled  = OBJ("w_6rd_relay").disabled = COMM_GetRadioValue("6rd_dhcp_option_rad")=="dhcp"?true:false;
	...
	InitWANDHCPValue: function()
	{
		...
		COMM_SetRadioValue("w_dhcp_dns_rad", "auto");
		...
		if(count > 0)
		{
			COMM_SetRadioValue("w_dhcp_dns_rad", "manual");
	...
	InitWANPPPoEValue: function()
	{
        ...
        COMM_SetRadioValue("pppoe_addr_type", "dynamic");
        OBJ("pppoe_ipaddr").value           = "";
        COMM_SetRadioValue("pppoe_sess_type", "share");
		...
		if(addrtype_wan1 == "ppp10")
		{
			...
			{
				COMM_SetRadioValue("pppoe_addr_type", "static");
			...
			if(dialup === "manual")		COMM_SetRadioValue("pppoe_reconnect_radio", "manual");
			else						COMM_SetRadioValue("pppoe_reconnect_radio", "alwayson");
	...
	InitWAN6RDValue: function()
	{
		...
		if(XG(this.wanact.inetp+"/ipv6/mode") == "6RD")
		{
			...
			{ 
				...
				COMM_SetRadioValue("6rd_dhcp_option_rad", "manual");
				COMM_SetRadioValue("6rd_dhcp_option_rad", "6rd_manual");
			}
			else
			{
				...
				COMM_SetRadioValue("6rd_dhcp_option_rad", "dhcp");
				COMM_SetRadioValue("6rd_dhcp_option_rad", "6rd_dhcp_option");
			}
	...
	InitWAN6TO4Value: function()
	{
		...
		else
		{
			...
			if(XG(this.rwan1+"/runtime/inf/inet/ipv4/ipaddr")!="")
			{
				var ip4addr = XG(this.rwan1+"/runtime/inf/inet/ipv4/ipaddr");
	...
	InitWANInfo: function()
	{
		...
		COMM_SetRadioValue("w_dhcp_dns_rad", "auto");
		OBJ("l_ipaddr").disabled = true;
		COMM_SetRadioValue("pppoe_addr_type", "dynamic"); //20100614
		COMM_SetRadioValue("pppoe_sess_type", "share"); //20100614
		COMM_SetRadioValue("6rd_dhcp_option_rad", "dhcp");
		if( this.wanact.svcsname != null )
		{
			/* init value */
		...
		switch(OBJ("wan_ipv6_mode").value)
		{
			...
			case "AUTODETECT":
				...
				*/
				...
				/* dns */
				var dnscnt = 0;
				if (COMM_GetRadioValue("w_dhcp_dns_rad")=="auto"?true:false)
				{
			...
			case "PPPOE":
				...
				if(OBJ("en_dhcp_pd").checked) 
				{
					if(COMM_GetRadioValue("pppoe_sess_type")=="share"?true:false)
					{
				...
				else 	 
				{
					...
					//XS(this.wanact.inetp+"/ipv6/mode", "AUTO");
           			...
           			if(COMM_GetRadioValue("pppoe_addr_type")=="dynamic"?true:false)
					XS(this.wanact.inetp+"/ipv6/mode", "AUTO");
				else
				{
					XS(this.wanact.inetp+"/ipv6/mode", "STATIC");
			...
			case "6RD":
				...
				if (COMM_GetRadioValue("6rd_dhcp_option_rad")=="manual"?true:false)
				{
					XS(this.wanact.inetp+"/ipv6/ipv6in4/rd/ipaddr", OBJ("w_6rd_prefix_1").value);
					XS(this.wanact.inetp+"/ipv6/ipv6in4/rd/prefix", OBJ("w_6rd_prefix_2").value);
					XS(this.wanact.inetp+"/ipv6/ipv6in4/rd/v4mask", OBJ("w_6rd_v4addr_mask").value);
...
*/
	Pre6TO4: function()
	{
		...
		{
    		BODY.ShowAlert("<?echo I18N('j', 'Invalid value for SLA ID');?>");
		...
		if(COMM_GetRadioValue("pppoe_sess_type")=="share"?true:false)
		{
				...
				XS(this.wan1.infp+"/lowerlayer", "");
           		...
           		else
           		{
                 		...
                 		if((COMM_GetRadioValue("w_dhcp_dns_rad")=="manual"?true:false) && OBJ("w_dhcp_pdns").value === "")
                 		{
			...
			*/
       			...
       			//var dialup = "ondemand";
			var dialup = "auto";
       			if(COMM_GetRadioValue("pppoe_reconnect_radio")=="alwayson"?true:false) dialup = "auto";
       			else if(COMM_GetRadioValue("pppoe_reconnect_radio")=="manual"?true:false) dialup = "manual";
	...
	{
		OBJ("pppoe_ipaddr").disabled = COMM_GetRadioValue("pppoe_addr_type")=="dynamic"?true:false;
		if(COMM_GetRadioValue("pppoe_addr_type")=="static"?true:false)
		{
			OBJ("box_lan_pd_body").style.display		= "none";
			OBJ("box_lan_auto").style.display 		= "";
			OBJ("box_lan_auto_body").style.display 		= "";
	...
	OnClickPppoeSessType: function()
	{
		if(COMM_GetRadioValue("pppoe_sess_type")=="share"?true:false)
		{
			COMM_SetRadioValue("pppoe_reconnect_radio", "alwayson");
