...
Page.prototype =
{
	...
	InitValue: function(xml)
	{
		...
		if (!this.macfp) { BODY.ShowAlert("<?echo I18N("j", "Init_MAC() ERROR!");?>"); return false; }
		...
		{		
			if(OBJ("mode").value == "DISABLE")
			{
				...
				<?
					if($FEATURE_NOSCH!="1")
				    {
						echo 'OBJ("sch_"+i).disabled             = true;\n';
						echo 'OBJ("schedule_"+i+"_btn").disabled = true;\n';
					}
				?>		
			...
			else
			{
				...
				<?
					if($FEATURE_NOSCH!="1")
				    {
						echo 'OBJ("sch_"+i).disabled             = false;\n';
						echo 'OBJ("schedule_"+i+"_btn").disabled = false;\n';
					}
				?>
			}
			...
			OBJ("client_list_"+i).value  = XG(b+"/mac");
			<?
			if($FEATURE_NOSCH!="1")
			{
	...
	PreSubmit: function()
	{
		...
		if(wps_enable && OBJ("mode").value != "DISABLE")
		{
		...
		*/
		...
		for (var i=1; i<=<?=$MAC_FILTER_MAX_COUNT?>; i+=1)
		{
			...
			if (OBJ("mac_"+i).value!=="")
			{
				...
				if(check_active==0 && OBJ("en_"+i).checked==0)
					check_active=1;
			}
		}
		
		if(cur_count==0 && OBJ("mode").value !== "DISABLE")
		{
			XS(this.macfp+"/policy", "DISABLE");
			BODY.ShowAlert("<?echo I18N("j", "No input rules. MAC filter will turn OFF!");?>");
		}
		if(cur_count!=0 && check_active==1)
		{
			BODY.ShowAlert("<?echo I18N("j", "Some rules are not active. MAC filter will ignore these rules and only run active rules!");?>");
		}
	...
	GetMAC: function(m)
	{
		...
		else if (m.length==12){				//change non_segment MAC address to normal type
			var m_tmp=m[0]+m[1]+":"+m[2]+m[3]+":"+m[4]+m[5]+":"+m[6]+m[7]+":"+m[8]+m[9]+":"+m[10]+m[11];
			m=m_tmp;
			var tmp=m.split(":");
		}
