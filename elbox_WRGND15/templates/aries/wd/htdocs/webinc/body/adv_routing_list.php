<tr>
	<td align="center">
		<input type=checkbox id="<? echo 'enable_'.$ROUTING_INDEX;?>" class="styled2" />
		<input type="hidden" id="<?echo "uid_".$ROUTING_INDEX;?>" value="<?=$ROUTING_INDEX?>"/>	
	</td>
	<td><input type=text id="<? echo 'name_'.$ROUTING_INDEX;?>" size=14 maxlength=13></td>
	<td>
		<input type=hidden id="<? echo 'metric_'.$ROUTING_INDEX;?>" size=3 maxlength=3>
			<select id="<?echo "inf_".$ROUTING_INDEX;?>" class="styled3">
...
<?		
			...
			*/
			...
			$i=1;
			//$i=3; ITR No.: 44611 UI- Static Routing display LAN-1 & LAN-2
			while ($i>0 && $i<4)
			{
				$ifname = "LAN-".$i;
				$ifpath = XNODE_getpathbytarget("runtime", "inf", "uid", $ifname, 0);
...
?>
	...
	<td><input type=text id="<? echo 'dstip_'.$ROUTING_INDEX;?>" size=16 maxlength=15></td>
	<td><input type=text id="<? echo 'netmask_'.$ROUTING_INDEX;?>" size=16 maxlength=15></td>
	<td><input type=text id="<? echo 'gateway_'.$ROUTING_INDEX;?>" size=16 maxlength=15></td>		
</tr>
