...
Authenticate.prototype =
{
	...
	Login_Hash: function(callback, user, passwd)
	{
		if (this.AuthorizedGroup >= 0)
		{
			if (callback) callback(null);
		...
		{
			self.Login_Send_Digest(callback, user, passwd, json);
		}
		AJAX.setHeader("Content-Type", "application/x-www-form-urlencoded");
	...
	Login_Send_Digest: function(callback, user, passwd, json)
	{
		var JsonData = eval('(' + json + ')');
		if (navigator.cookieEnabled) document.cookie = "uid="+JsonData.uid+"; path=/";
		var key = passwd;
		var mix = user + JsonData.CHALLENGE;
		var digest = hex_hmac_md5(key, mix);
		var DIGEST = digest.toUpperCase();
		var payload = "USERNAME="+user+"&DIGEST="+DIGEST;
		var AJAX = GetAjaxObj("login_send_digest");
		AJAX.release();
		AJAX.createRequest();
		AJAX.onCallback = function(json)
		{
		...
		AJAX.setHeader("Content-Type", "application/x-www-form-urlencoded");
		AJAX.returnXml = false;//return JSON data
		AJAX.sendRequest("authentication.cgi", payload);
	},	
