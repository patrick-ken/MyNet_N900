...
function COMM_EatLeftRightSpace(str)
{
	str = str.replace(/(^\s*)|(\s*$)/g, "");
	return str;
}

function COMM_CheckSpace(str)
{
	var i=0;
	var result=0;
...
function COMM_SetRadioValue(name, value)
{
	...
	NewCheckRadio.refresh(); // Refresh the WD style input.
}
...
function COMM_EscapeHTMLSC(str)
{
	...
	/* DO NOT escape the space, which will cause non-breaking message. */
	//str = str.replace(/ /g,"&nbsp;");
	return str;
}
...
function COMM_GoToWebPageByPOST(URL, PARAMS)
{
	/* PARAMS can implement with an array */
	var temp=document.createElement("form");
	temp.action=URL;
	temp.method="POST";
	temp.style.display="none";/* hide dynamic form */
	for(var x in PARAMS)
	{
...
function COMM_WDButtonClassSet(objID,SetClass,BackgroundIMG)
{
	if(document.getElementById(objID)!="" && document.getElementById(objID)!="undefined" && document.getElementById(objID)!=null )
	{
		document.getElementById(objID).className = SetClass;
		document.getElementById(objID).style.backgroundImage = "url('/pic/bg_"+BackgroundIMG+".png')";
	}
}

function COMM_DelayTime(millis)
{
	var date = new Date();
	var curDate = null;
	curDate = new Date();
...
function COMM_AddSelectOption(objID,objNAME,objVALUE)
{
	selt = document.getElementById(objID);
	if(selt!="" && selt!="undefined" && selt!=null )
	{
		//var new_select_option = new Option(objNAME,objVALUE);
		//selt.options.add(new_select_option);
		var new_select_option=document.createElement("option");
		new_select_option.text = objNAME;
		new_select_option.value = objVALUE;
		try
		{
			// for IE 8 before
			selt.add(new_select_option,selt.options[null]);
		}
		catch(e)
		{
			selt.add(new_select_option,null);
		}
	}
}