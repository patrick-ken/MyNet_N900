...
else
{
	...
	$_GLOBALS["FATLADY_message"] = i18n("Unsupported log level is assigned");	/* internal error, no i18n. */
}
...
function check_syslog_setting($path, $addrtype, $lan_ip, $mask)
{
	if (query($path."/enable") == "1")
	{
		...
		if ($hostid == "")
		{
			set_result("FAILED", $path."/ipv4/ipaddr", i18n("SYSLOG host can not be empty."));
			return "FAILED";
		}
		if ($hostid <=0)
		{
