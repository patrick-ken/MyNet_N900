...
function PHYINF_getdevdatamac2($name)
{
	/* we allow return null mac, for case device never initialized (never mfc init) */
	$mac = query("/runtime/devdata/".$name);
	return $mac;
}
...
function PHYINF_gettargetmacaddr($mode, $ifname)
{
	...
	if ($mac=="")
	{
		...
		{	/* Only 1 interface, use WAN MAC address */
			if		($ifname=="ETH-1") $mac = PHYINF_getdevdatamac2("wanmac");
		}
		else if ($mode=="1W1L")
		{	/* 2 interfaces, ETH-1 is LAN and ETH-2 is WAN */
			if      ($ifname=="ETH-1") $mac = PHYINF_getdevdatamac2("lanmac");
			else if ($ifname=="ETH-2") $mac = PHYINF_getdevdatamac2("wanmac");
		}
		else if ($mode=="1W2L")
		{	/* 3 interfaces, ETH-1 is LAN1, ETH-2 is LAN2 and ETH-3 is WAN. */
			if      ($ifname=="ETH-1") $mac = PHYINF_getdevdatamac2("lanmac");
			else if ($ifname=="ETH-2") $mac = PHYINF_getdevdatamac2("lanmac2");
			else if ($ifname=="ETH-3") $mac = PHYINF_getdevdatamac2("wanmac");
		}
	}
	return tolower($mac);
}
