...
{
	if ($uid=="BAND24G-1.1")	return "ath2";
	else if ($uid=="BAND24G-1.2") return "ath3";
	else if ($uid=="BAND5G-1.1") return "ath0";
	else if ($uid=="BAND5G-1.2") return "ath1";
	else if ($uid=="STATION24G-1.1") 	return "ath2";
	else if ($uid=="STATION5G-1.1") 	return "ath0";
	return "ath0";
}
	...
	while ($i>0)
	{
		...
		else
		{
			...
			if(isguestzone($uid)=="1")
			{
				...
				$y=$i-1;
				$uid1 		= $prefix.$y;
				$p 		= XNODE_getpathbytarget("", "phyinf", "uid", $uid1, 0);
			}
			...
			$puren			= "0";
			$pureg			= "0";
			$wds			= query("wds/enable");                  if ($wds!="1")                          {$wds="0";}	
			...
			if($wds==1)	{	$makeVAPcmd = "/etc/ath/makeVAP ap-wds \"".$ssid."\" \"".$params."\"";	}
			else {	$makeVAPcmd = "/etc/ath/makeVAP ap \"".$ssid."\" \"".$params."\"";	}
...
function wificonfig($uid)
{
	...
	if(host_guest_dependency_check($prefix)==0)	return error(8);
