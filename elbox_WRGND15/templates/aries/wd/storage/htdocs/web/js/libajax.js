...
HTTPClient.prototype =
{
	...
	requestAsyn : true,
	returnXml : true,
	isWDOrion : false, /* added by alex shi for WD Orion remote access, we need to handle the xml in the packet when HTTP return status is not 200 OK */ 
	__header : null,
	onSend : null,
	onCallback : null,
	...
	__callback : function()
	{
		...
		{
			if (this.__httpRequest!=null) /* added by alex shi to avoid to get the status of httpRequest at __httpRequest is null */  
		...
		else
		{
			...
			{
				/* add http request status 201 for WD Orion remote access service */
				if (this.__httpRequest.status == 200 || this.__httpRequest.status == 201)
				{
					if (this.onCallback)
					{
						if (this.returnXml)
				...
				else
				{
					if (this.isWDOrion)
					{
                        this.onError(this.__httpRequest.responseText);
					}
					else
					{
						this.onError("Error : Request return error("+ this.__httpRequest.status +").");
...
/*			//For IOS 6 Safari browser, Ajax with POST action would cache.
	...
	setHeader : function(header, value)
	{
		if (header && value)
		{
			...
			var isAdded = 0;
			for(var i=0;i<this.__header.length;i++)
			{
				if(this.__header[i].name=="Cache-Control" && this.__header[i].name=="no-cache")
				{
					isAdded = 1;
					break;
				}
			...
			{
				var tmpHeader = new Object();
				tmpHeader.name = "Cache-Control";
				tmpHeader.value = "no-cache";
				this.__header[ this.__header.length ] = tmpHeader;
			}
		}
	},
