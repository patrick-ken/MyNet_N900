...
startcmd("adduser -D -H -u 0 -G root root");
/* Create nobody */
startcmd("addgroup -g 500 nobody");
startcmd("adduser -D -H -u 500 -G nobody nobody");
/* group name shortcut */
$GROUP_0	= "root";
$GROUP_500	= "nobody";
...
foreach ("/device/account/entry")
{
	...
	fwrite("a", $hnapf, $name.":".$passwd."\n");
}
/* Add the account for Samba, DLNA, ITune, FTP and AFP */
$storage_name = get("s","/wd/storage/username");
$storage_name1 = "\"".$storage_name."\"";
$storage_pwd = query("/wd/storage/password");
$storage_dir = "/tmp/ftp_share";
startcmd("adduser -h ".$storage_dir." -D ".$storage_name1);
fwrite("a", $pwdf, $storage_name.":".$storage_pwd."\n");
