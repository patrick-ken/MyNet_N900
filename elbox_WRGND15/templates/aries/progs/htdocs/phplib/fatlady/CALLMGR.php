...
//TRACE_debug("FATLADY: busy_number".$number);
		if($number == "")
		{
			set_result("FAILED", $FATLADY_prefix."/runtime/callmgr/voice_service:1/mobile/call_forward/busy_number", i18n("The Busy Number cannot be empty."));
			$rlt = "-1";
		}
		else
		{
...
//TRACE_debug("FATLADY: no_reply_number".$number);
		...
		else
		{
				...
				if(isdigit($string)=="0")
				{
					set_result("FAILED", $FATLADY_prefix."/runtime/callmgr/voice_service:1/mobile/call_forward/no_reply_number", i18n("Invalid value for the No Answer Number"));
					$rlt = "-1";
				}
			}
			else
		...
		if($time == "")
		{
			set_result("FAILED", $FATLADY_prefix."/runtime/callmgr/voice_service:1/mobile/call_forward/no_reply_time", i18n("The No Answer Timer cannot be empty."));
			$rlt = "-1";
		}
		else
		{
