...
function check_bwc($prefix)
{
	foreach($prefix."/bwc/entry")
	{
        ...
        {
             set_result("FAILED", $prefix."/entry:".$bwc_index."/bandwidth", i18n("The input bandwidth is not a digit."));
             return "FAILED";
        }
        $flag = query("flag");
        ...
        foreach($prefix."/bwc/entry:".$bwc_index."/rules/entry")
        {
            ...
            if( $en!="" && $en!="0" && $en!="1")
            {
	             set_result("FAILED", $prefix."/entry:".$bwc_index."/rules/entry:".$InDeX."/enable", i18n("The input rules disabled are not digital numbers."));
    	         return "FAILED";
			}
