		...
		if ( $gw != "" )
		{
			...
			if( $inf_addrtype != "" && $inf_addrtype == "ipv4")
			{
				...
				if (ipv4networkid($inf_addr,$inf_mask) != ipv4networkid($gw,$inf_mask))
					return set_result("FAILED", $entry."/via",
						i18n("Invalid gateway, the gateway should be in the same network of the WAN interface."));

				if (ipv4hostid($inf_addr,$inf_mask) == ipv4hostid($gw,$inf_mask))
					return set_result("FAILED", $entry."/via",
						i18n("Invalid gateway; the IP address of the gateway shouldn't have the same IP as the device."));
			}
		}
		$i = 1;
		while ($i < $InDeX)
