...
function check_dhcp_config($path, $inetp)
{
	...
	{
		set_result("FAILED",$path."/start",i18n("The input range of the host ID is not digital."));
		return;
	}
	if (isdigit($end_ip) == 0)
	{
		set_result("FAILED",$path."/end",i18n("The input range of the host ID is not digital."));
		return;
	}
	...
	if ($start_ip > $end_ip)
	{
		set_result("FAILED",$path."/start",i18n("The start host ID of the lease pool should be small than the end host ID."));
		return;
	}
	...
	if ($start_ip == 0)
	{
		set_result("FAILED",$path."/start",i18n("The input range of the host ID is out of the boundary."));
		return;
	}
	if ($end_ip >= ipv4maxhost($mask))
	{
	//	set_result("FAILED",$path."/end",i18n("The input range of the host ID is out of the boundary."));
	//	return;
	}
	...
	{
		set_result("FAILED",$path."/start",i18n("The input range of the host ID is include the LAN IP address."));
		//return;
	}
	...
	{
		set_result("FAILED",$path."/leasetime",i18n("The input lease time is not a digit."));
		return;
	}
	...
	{
		set_result("FAILED",$path."/leasetime",i18n("The input lease time can not be less than 1 hour."));
		return;
	}
	...
	foreach ($p)
	{
		...
		while ($i2 < $cnt)
		{
			...
			{
				set_result("FAILED", $p.":".$InDeX."/macaddr", i18n("Duplicated MAC addresses"));
				return;
			}
		}
   		...
   		while ($i <= $cnt)
    	{
        	...
        	{
           		set_result("FAILED", $dhcpp."/staticleases/entry:".$i."/macaddr", i18n("Duplicated MAC addresses"));
            	$rlt = "-1";
            	break;
        	}
        	...
        	{
           		set_result("FAILED", $dhcpp."/staticleases/entry:".$i."/hostid", i18n("Duplicated IP addresses"));
            	$rlt = "-1";
            	break;
        	}
        	$i++;
