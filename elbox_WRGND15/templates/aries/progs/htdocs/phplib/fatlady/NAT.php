...
function verify_portforward($path, $max)
{
	...
	foreach ($path."/entry")
	{
		...
		if ($start=="")
			return result("FAILED", $entry."/external/start", i18n("Please enter the public port."));
...
function verify_porttrigger($path, $max)
{
	...
	foreach ($path."/entry")
	{
		...
		if ($start=="")
			return result("FAILED", $entry."/trigger/start", i18n("Please enter the application port."));
		if (isvalid_port($start)!="YES")
