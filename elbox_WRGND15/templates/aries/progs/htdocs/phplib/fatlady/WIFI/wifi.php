...
function check_wep($p)
{
	...
	if ($err_i > 0)
	{
		set_result("FAILED", $p."/wep/defkey", i18n("The default WEP key index should be between 1 and 4."));
		return 0;
	}
	if ($err_kl > 0)
	{
...
function check_radius($p)
{
	...
	if ($len > 64 || $len < 1)
	{
		set_result("FAILED", $p."/eap/secret", i18n("The length of secret should be between 1 and 64."));
		return 0;
	}
	if (isprint($value)!=1)
	{
...
function check_psk($p)
{
	...
	$len = strlen($key);
	$key_first = substr($key,0,1);
	$key_last = substr($key,$len-1,1);
	if ($type == "")
	{
		...
		{
			set_result("FAILED", $p."/psk/key", i18n("The length of PSK / Passphrase should be between 8 and 64."));
			return 0;
		}
	}
	if ($type==0)
	...
	else if ($key_first==" " || $key_last==" ")	//denotes the space between words
	{
		set_result("FAILED", $p."/psk/key", i18n("The PSK space words should be between words."));
		return 0;
	}
	if ($gtk!="")
	{
...
function check_acl($p)
{
	...
	foreach ($p."/acl/entry")
	{
		...
		if ($err > 0)
		{
			set_result("FAILED", $p."/acl/entry:".$InDeX, i18n("Duplicated MAC address"));
			return 0;
		}
	}
...
function check_media($p)
{
	...
	$meet	= 0;
	if ($wlmode == "b" || $wlmode == "g" || $wlmode == "bg" || $wlmode == "a" ||
		$wlmode == "n" || $wlmode == "bn" || $wlmode == "gn" || $wlmode == "bgn" ||$wlmode == "an") $meet++;
	...
	if ($v < 20 || $v > 1000)
	{
		set_result("FAILED", $p."/beacon", i18n("Invalid value of Beacon. The range should be between 20 and 1000."));
		return 0;
	}
	/* fragthresh */
	$v = query($p."/fragthresh");
	$mod = $v%2;
	if ($v < 256 || $v > 2346 || $mod!=0)
	{
		set_result("FAILED", $p."/fragthresh", i18n("Invalid value of Fragmentation Threshold. The range should be an even number between 256 and 2346."));
		return 0;
	}
	/* rtsthresh */
	$v = query($p."/rtsthresh");
	if ($v < 256 || $v > 2346)
	{
		set_result("FAILED", $p."/rtsthresh", i18n("Invalid value of RTS Threshold. The range should be between 256 and 2346."));
		return 0;
	}
	/* ctsmode */
	/* dtim */
	...
	{
		set_result("FAILED", $p."/dtim", i18n("Invalid value of DTIM. The range should be between 1 and 255."));
		return 0;
	}
	/* channel */
	$v = query($p."/channel");
	...
	/* tx power */
	$v = query($p."/txpower");
	if ($v!="100" && $v!="50" && $v!="25" && $v!="12.5" && $v!="70" && $v!="35" && $v!="15")
	{
...
function fatlady_wifi($prefix, $phyinf)
{
	...
	if ($value=="STA")
	{		
	  //$p1 = XNODE_getpathbytarget($prefix, "phyinf", "uid", "WLAN-1");  // original
	  $p1 = XNODE_getpathbytarget($prefix, "phyinf", "uid", $phyinf);  // jack add		
		...
		//TRACE_debug("$phyinf=".$phyinf."$p1=".$p1." $p=".$p);
