...
function verify_dhcps4($path)
{
	...
	if ($start > $end)
		return result("FAILED", $path."/start",
				i18n("The start of the DHCP lease pool should be less than or equal to the end address."));
	...
	foreach("wins/entry")
	{
		if ($InDeX>$cnt) break;
		if (ipv4networkid($VaLuE, 32)=="")
			return result("FAILED", $path."/wins/entry:".$InDeX, i18n("The WINS address is invalid."));
	}
