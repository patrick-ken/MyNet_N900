...
function check_ppp4($path)
{
	...
	if ($mtu != "")
	{
		...
		{
			set_result("FAILED",$path."/mtu",
				i18n("The MTU value is too large; the valid value for pptp is between 576 and 1400."));
			return;
		}
		else if ($over=="l2tp" && $mtu > 1400)
		{
			set_result("FAILED",$path."/mtu",
				i18n("The MTU value is too large; the valid value for 12tp is between 576 and 1400."));
			return;
		}
		else if ($mtu > 1492)
		{
            if($over == "tty")
            {
                ...
                {
                    set_result("FAILED",$path."/mtu",
                            i18n("The MTU value is too large; the valid value for 3G is between 576 and 1500."));
                    return;
                }
            }
            else
            {
                set_result("FAILED",$path."/mtu",
                    i18n("The MTU value is too large; the valid value is between 576 and 1492."));
                return;
            }
		}
	...
	if ($mru != "")
	{
		...
		if ($mru < 576)
		{
			set_result("FAILED",$path."/mru",
				i18n("The MRU value is too small; the valid value mustn't be smaller than 576."));
			return;
		}
		if ($mru > 1492)
		{
			set_result("FAILED",$path."/mru",
				i18n("The MRU value is too large; the valid value is between 576 and 1492."));
			return;
		}
	...
	if ($tout != "")
	{
		if (isdigit($tout)=="0" || $tout < 0 || $tout > 10000)
		{
			set_result("FAILED",$path."/dialup/mode",
				i18n("Invalid value for idle timeout; the valid value is between 0 and 10000."));
			return;
		}
	}
