	...
	foreach ($path."/entry")
	{
		...
		if (query("provider")!="ORAY")
		{
			if (query("hostname")=="")
				return set_result(
						...
						i18n("Please enter the host name.")
						);
		...
		if (query("username")=="")
			return set_result(
						...
						i18n("Please enter the user account or e-mail.")
						);
		if (query("password")=="")
			return set_result(
						"FAILED",
		...
		if ($per_err > 0)
			return set_result(
						...
						i18n("Invalid period. The range of Timeout is between 1 and 8670.")
						);
	}
	return "OK";
}
...
if (query($FATLADY_prefix."/inf/ddns4")!="")
{
