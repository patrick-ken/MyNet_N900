...
function check_wep($p)
{
	...
	while ($i < 1)
	{
		...
		if ($ascii=="")
		{
			...
			{
				if ($len == 5) $ascii = 1;
				else if ($len == 10) $ascii = 0;
				else    {$err_s++; break;}
			}
			else if ($size == 128)
			{
				if		($len == 13) $ascii = 1;
				else if ($len == 26) $ascii = 0;
				else	{$err_s++; break;}
			}
			else
			{
	...
	if ($err_s > 0)
	{
        if ($size == 64)
        {
            return result("FAILED", $p."/wep/size", i18n("The WEP key must be 5 characters or 10 HEX characters long."));
        }
        else if ($size == 128)
        {
            return result("FAILED", $p."/wep/size", i18n("The WEP key must be 13 characters or 26 HEX characters long."));
        }
        else
        {
        return result("FAILED", $p."/wep/size",
                i18n("The size of the WEP key must be 64 or 128 bits.")." ".
                i18n("The WEP key should be 5, 10, 13, or 26 characters long."));
        }
	}
	if ($err_i > 0)
		return result("FAILED", $p."/wep/defkey",
				i18n("The default WEP key index should be between 1 and 4."));
	if ($err_kl > 0)
	...
	if ($err_km > 0)
	{
		if ($size == 64)
    	{
				return result("FAILED", $p."/wep/ascii",i18n("The WEP key must be 5 characters or 10 HEX characters long."));
		}
		if ($size == 128)
    	{
				return result("FAILED", $p."/wep/ascii",i18n("The WEP key must be 13 characters or 26 HEX characters long."));
		}
	}
	...
	if($val!="")
	{
		if (isdigit($val)!=1 || $val > 65535 || $val < 30)
			return result("FAILED", $p."/wpa/groupintv",
					i18n("The value of the group key update interval should be between 30 and 65535."));				
	}
	if (query($p."/eap#")==0)
	...
	foreach($p."/eap")
	{
		...
		if (isdigit($val)!=1 || $val > 65535 || $val < 1)
			return result("FAILED", $p."/eap:".$InDeX."/port",
				i18n("The value of the port should be between 1 and 65535. (Recommended port : 1812)"));
		$val = query($p."/eap:".$InDeX."/secret");
	$len = strlen($val);
	if ($len > 64 || $len < 1)
			return result("FAILED", $p."/eap:".$InDeX."/secret",
				i18n("The length of the secret code should be between 1 and 64."));
	if (isprint($val)!=1)
			return result("FAILED", $p."/eap:".$InDeX."/secret",
...
function check_wapias($p)
{
	...
	if ($val > 65535 || $val < 1)
		return result("FAILED", $p."/wapi/port",
				i18n("The value of the port should be between 1 and 65535."));
...
function check_psk($p)
{
	...
	{
		return result("FAILED", $p."/psk/key",
				i18n("The PSK should be between 8 and 63 printable characters."));
	}
	if ($gtk!="")
	{
		if (isdigit($gtk)!=1)
			return result("FAILED", $p."/rekey/gtk",
					i18n("The value of the rekey interval should be a number."));
	}
...
function check_wps($p)
{
	...
	$val = query($p."/wps/locksecurity");
	if ($val != 1) set($p."/wps/locksecurity", 0);
...
function check_acl($p)
{
	...
	foreach ($p."/acl/entry")
	{
		...
		if ($err > 0)
			return result("FAILED", $p."/acl/entry:".$InDeX, i18n("Duplicated MAC address"));
	}
...
function check_media($band, $p)
{
	...
	if ($v<20 || $v>1000)
		return result("FAILED", $p."/beacon",
				...
				i18n("The value should be between 20 and 1000."));
	/* fragthresh */
	...
	if ($v<1500 || $v>2346 || $mod!=0)
		return result("FAILED", $p."/fragthresh",
					...
					i18n("The value should be an even number between 1500 and 2346."));
	/* rtsthresh */
	$v = query($p."/rtsthresh");
	...
	if ($v<256 || $v>2346)
		return result("FAILED", $p."/rtsthresh",
					...
					i18n("The value should be between 256 and 2346."));
	/* ctsmode */
	/* dtim */
	$v = query($p."/dtim");
	...
	if (isdigit($v)!=1 || $v < 1 || $v > 255)
		return result("FAILED", $p."/dtim",
					...
					i18n("The value should be between 1 and 255."));
	/* channel */
	$v = query($p."/channel");
	...
	if ($v!="20" && $v!="20+40" && $v!="80") set($p."/dot11n/bandwidth", "20");
	/* dot11n/coexist */
