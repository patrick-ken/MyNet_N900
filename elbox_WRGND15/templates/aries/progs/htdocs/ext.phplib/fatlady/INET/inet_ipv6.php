...
function check_static($path, $needgw)
{
	...
	if (isdigit($prefix)=="0")
		return result("FAILED", $path."/prefix", i18n("Prefix value must be a digit."));
	...
	if ($routerlft!="")
	{
		if (isdigit($routerlft)=="0")
			return result("FAILED", $path."/routerlft", i18n("Lifetime value must be a digital number."));
	...
	if ($preferlft!="")
	{
		if (isdigit($preferlft)=="0")
			return result("FAILED", $path."/preferlft", i18n("Lifetime value must be a digital number."));
		if ($preferlft < 0)
	...
	if ($validlft!="")
	{
		if (isdigit($validlft)=="0")
			return result("FAILED", $path."/validlft", i18n("Lifetime value must be a digital number."));
		if ($validlft < 0)
			return result("FAILED", $path."/validlft", i18n("Invalid valid lifetime value"));
	}

	if ($preferlft!="" && $validlft=="")
		return result("FAILED", $path."/validlft", i18n("Don't leave valid lifetime blank."));
	if ($preferlft=="" && $validlft!="")
...
function check_ipv6($path, $needgw)
{
	...
	if($pdhint_enable=="1")
	{
		...
		if (isdigit($pdhint_prefix)=="0")
			return result("FAILED", $path."/pdhint/prefix", i18n("Prefix value must be a digit."));
		...
		if ($pdhint_plft!="")
		{
			if (isdigit($pdhint_plft)=="0")
				return result("FAILED", $path."/pdhint/preferlft", i18n("Lifetime value must be a digital number."));
			if ($pdhint_plft < 0)
				return result("FAILED", $path."/pdhint/preferlft", i18n("Invalid preferred lifetime value"));
		}
		else
		{
			return result("FAILED", $path."/pdhint/preferlft", i18n("No preferable lifetime value"));
		}
		...
		if ($pdhint_vlft!="")
		{
			...
			if (isdigit($pdhint_vlft)=="0")
				return result("FAILED", $path."/pdhint/validlft", i18n("Lifetime value must be a digital number."));
			if ($pdhint_vlft < 0)
				return result("FAILED", $path."/validlft", i18n("Invalid valid lifetime value"));
		}
	}
	...
	if ($mtu!="")
	{
		...
		if ($mtu<1280)
			return result("FAILED", $path."/mtu",
				i18n("The MTU value is too small; the valid value is between 1280 and 1492."));
		if ($mtu>1500)
			return result("FAILED", $path."/mtu",
				i18n("The MTU value is too large; the valid value is between 1280 and 1492."));
	}
