...
//		if ($delta > 255)				return i18n("The IP address range is too large.");
	...
	if ($inf0 == "LAN")
	{
		...
		if($ipaddr1==$lanip)	return i18n("The start IP address can't be the same as the device IP.");
...
//			return i18n("The start IP address and router address should be in the same network subnet.");
		if($ipaddr2!="")
		{
			if($ipaddr2==$lanip)	return i18n("The end IP address can't be the same as the device IP.");
...
function verify_setting($path)
{
	...
	foreach ($path."/entry")
	{
		...
		/* 5. protocol */
		$prot = query("protocol");
		if ($prot!="ALL" && $prot!="TCP" && $prot!="UDP" && $prot!="TCP+UDP" && $prot!="ICMPv6")
		{
