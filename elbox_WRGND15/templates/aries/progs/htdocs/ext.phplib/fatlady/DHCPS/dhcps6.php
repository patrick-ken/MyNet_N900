		...
		if($pdmode=="0") /* generic */
		{
			...
			if ($pdnetwork!="")
			{
				...
				return result("FAILED", $path."/pd/prefix",
						i18n("The pd prefix should be a decimal number."));
			}
			...
			if (isdigit($pdstart)==0)
				return result("FAILED", $path."/pd/start",
					i18n("The start of the DHCP-PD lease pool should a decimal number."));
			if (isdigit($pdcount)==0)
				return result("FAILED", $path."/pd/count",
					i18n("The count of the DHCP-PD lease pool should a decimal number."));
			//if ($pdcount >= 4294967296)
			if ($pdcount >= 256)
				return result("FAILED", $path."/pd/count",
					i18n("The limit of DHCP-PD count is 255."));
