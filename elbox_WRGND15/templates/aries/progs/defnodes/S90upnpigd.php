...
$sn			= query("/runtime/device/serial_number");	// Modified from "None" , by Jerry Kao.
$hostname	= query("/device/hostname");	// michael_lee

if (query("/device/layout") == "bridge" )	return;
...
set($desc_root."/device",		"");	anchor($desc_root."/device");
	...
	set("modelName",			$model);
	set("modelNumber",			$model);		/* Modified from "1" to "DIR-645", by jerry_kao, 110810. */
	set("modelURL",				$model_url);
	set("serialNumber",			$sn);
	set("UDN",					$udn);
	set("rx_delay",				"80000");		/*Add delay for IGD rx_bytes counter updated in PRO & STORAGE */
	set("tx_delay",				"80000");		/*Add delay for IGD rx_bytes counter updated in PRO & STORAGE */
