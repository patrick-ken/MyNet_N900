...
if ($SEARCH_TARGET == "ssdpall")
{
	foreach ($path)
	{
		...
		{
			//TRACE_debug("M-SERCH.php [ssdpall] dtype=[".$dev_type."]");
...
if ($SEARCH_TARGET == "rootdevice")
{
	...
	foreach ($path)
	{
		...
		/* Only response the supported device type,
		   from /inf:x/upnp/entry (defined in defnodes/defaultvalue.xml)	 
		*/		
		if (dtype_check($dev_type, $inf_path)=="1")
		{
			//TRACE_debug("M-SERCH.php [rootdevice] dtype=[".$dev_type."]");
...
if ($SEARCH_TARGET == "uuid")
{
	foreach ($path)
	{
		...
		if (dtype_check($dev_type, $inf_path)=="1")
		{
			//TRACE_debug("M-SERCH.php [uuid] dtype=[".$dev_type."]");
...
if ($SEARCH_TARGET == "devices")
{
	foreach ($path)
	{
		...
		if (dtype_check($dev_type, $inf_path)=="1")
		{
			//TRACE_debug("M-SERCH.php [devices] dtype=[".$dev_type."]");
...
if ($SEARCH_TARGET == "services")
{
	foreach ($path)
	{
		...
		if (dtype_check($dev_type, $inf_path)=="1")
		{
			//TRACE_debug("M-SERCH.php [service] dtype=[".$dev_type."]");
