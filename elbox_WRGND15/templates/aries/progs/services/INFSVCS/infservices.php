...
function infsvcs_setup($name, $cfg, $sts, $ipv6)
{
    ...
    $next   = query("infnext");
	$child  = query("child");
	$childgz  = query("childgz");//child for guest zone
	$capwap = query("capwap"); //sandy add
...
function infsvcs_bridge($index)
{
	...
	else
	{
		stopcmd("service IPTDEFCHAIN restart");
		stopcmd("service MULTICAST restart");
		stopcmd("service CAPWAP restart");
	}
	...
	else
	{
		...
		startcmd("service MULTICAST restart");
		startcmd("service DEVICE.TIME start");
		startcmd("service CAPWAP restart");
