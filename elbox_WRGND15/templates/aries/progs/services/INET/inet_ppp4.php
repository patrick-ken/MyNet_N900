...
/* bouble - workaround for pro/storage for igmpproxy */
$model		= query("/runtime/device/modelname");
if($model == "MyNetN900" || $model == "MyNetN900C" || $model == "MyNetAC1800")	{	$workaround  = 1;	}
else													{	$workaround  = 0;	}

if ( $workaround == 1 )
{
	echo 'event MULTICAT_RESTART add "service MULTICAST restart"\n';
}
...
fwrite(a, $STOP,
	...
	'sleep 2\n'.
	'RETRY_COUNT=0\n'.
	'KILL_IGMPPROXY='.$workaround.'\n'.
	'while [ -f '.$sfile.' ]; do\n'.
	'	echo "[$0]: '.$sfile.' still exist, kill again and wait 2 second ..." > /dev/console\n'.
	'	/etc/scripts/killpid.sh '.$dialuppid.'\n'.
	'	/etc/scripts/killpid.sh '.$pppd_pid.'\n'.
	'	sleep 2\n'.
	'	if [ "$KILL_IGMPPROXY" = "1" ]; then\n'.
	'		RETRY_COUNT=`expr $RETRY_COUNT + 1`\n'.
	'		echo "#RETRY_COUNT=$RETRY_COUNT" > /dev/console\n'.
	'		if [ "$RETRY_COUNT" -gt "4" ]; then\n'.
	'			killall igmpproxy\n'.
	'			sleep 3\n'.
	'			xmldbc -t "multicase_restart:10:event MULTICAT_RESTART"\n'.
	'		fi\n'.
	'	fi\n'.
	'done\n'
	);
