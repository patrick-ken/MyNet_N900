...
function inet_ipv6_static($inf, $devnam, $inetp)
{
	...
	anchor($inetp."/ipv6");
	/* Add to determine the IPv6 is ready or not. 
	   Get the link local address by phyinf
	*/
...
function inet_ipv6_autodetect($inf, $infp, $ifname, $phyinf, $stsp, $inetp) 
{
	...
	if($V4isPPP==1 || $v4mode=="ppp10")
	{
		...
		/* detect PPPOE server */
		/*
		startcmd('sh /etc/events/WAN_ppp_dis.sh '.$v4actuid.' START');
		startcmd('sleep 10');
		...
		*/
		...
		startcmd('if [ "$result" == "PPPoE" ]; then');
		startcmd('echo AUTODETECT change to ppp10 mode > /dev/console');
		startcmd('xmldbc -s '.$v4infp.'/infprevious "'.$inf.'"'); //Don't start when system reboot
		startcmd('xmldbc -s '.$v4infp.'/child "'.$llinf.'"');
		startcmd('xmldbc -s '.$v4inetp.'/addrtype "ppp10"');
		startcmd('xmldbc -s '.$v4inetp.'/ppp6/username "'.$username.'"');
		startcmd('xmldbc -s '.$v4inetp.'/ppp6/password "'.$password.'"');
		startcmd('xmldbc -s '.$v4inetp.'/ppp6/dialup/mode "'.$dialupmode.'"');
		startcmd('xmldbc -s '.$v4inetp.'/ppp6/over "eth"');
		startcmd('xmldbc -s '.$llinfp.'/infnext "'.$v6actuid.'"');
		startcmd('xmldbc -s '.$llinfp.'/inet ""');
		startcmd('xmldbc -s '.$v6actinfp.'/infprevious "'.$llinf.'"');
		startcmd('xmldbc -s '.$v6actinfp.'/infnext "'.$next.'"');
		startcmd('xmldbc -s '.$v6actinfp.'/defaultroute "0"');
		startcmd('xmldbc -s '.$v6actinfp.'/active "1"');
		//startcmd('xmldbc -s '.$infp.'/child "LAN-4"');
		startcmd('xmldbc -s '.$v6actinfp.'/child "'.$child.'"');
		startcmd('xmldbc -s '.$v6actinetp.'/ipv6/mode "AUTO"');
		startcmd('xmldbc -s '.$v6actinetp.'/ipv6/dhcpopt "IA-NA+IA-PD"');
		startcmd('xmldbc -s '.$v6actinetp.'/ipv6/dns/entry:1 "'.$pdns.'"');
		startcmd('xmldbc -s '.$v6actinetp.'/ipv6/dns/entry:2 "'.$sdns.'"');
		startcmd('xmldbc -s '.$v6actinetp.'/ipv6/dns/count "'.$dnscnt.'"');
		startcmd('service INET.'.$v4actuid.' restart');
		stopcmd('service INET.'.$v4actuid.' stop');
		startcmd('else');
		startcmd('xmldbc -s '.$v6actinfp.'/infprevious "'.$inf.'"'); //Don't start when system reboot
		startcmd('xmldbc -s '.$v6actinfp.'/child "'.$child.'"');
		startcmd('xmldbc -s '.$v6actinfp.'/infnext "'.$next.'"');
		startcmd('xmldbc -s '.$v6actinetp.'/ipv6/mode "AUTO"');
		startcmd('xmldbc -s '.$v6actinetp.'/ipv6/dns/entry:1 "'.$pdns.'"');
		...
		startcmd('xmldbc -s '.$v6actinetp.'/ipv6/dns/count "'.$dnscnt.'"');
		startcmd('echo AUTODETECT change to AUTO mode > /dev/console');
		startcmd('service INET.'.$v6actuid.' restart');
		stopcmd('service INET.'.$v6actuid.' stop');
		startcmd('fi');
	}
	else
	{
		...
		startcmd('event WAN.RESTART');
		*/
		set($v6actinfp."/infprevious", $inf);
		set($v6actinfp."/infnext", $next);
		set($v6actinfp."/defaultroute", "1");
		set($v6actinfp."/child", $child);
		set($v6actinetp."/ipv6/mode", "AUTO");
		set($v6actinetp."/ipv6/dhcpopt", "IA-NA+IA-PD");
		set($v6actinetp."/ipv6/dns/entry:1", $pdns);
		set($v6actinetp."/ipv6/dns/entry:2", $sdns);
		set($v6actinetp."/ipv6/dns/count",   $dnscnt);
		startcmd('# Change to AUTO mode');
		startcmd('echo AUTODETECT change to AUTO mode > /dev/console');
		startcmd('service INET.'.$v6actuid.' restart');
		stopcmd('service INET.'.$v6actuid.' stop');
